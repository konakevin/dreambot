...
- [ ] 🔴 **CRITICAL (Kevin, 2026-09-04): album posts on the HOME feed show as a single card with NO album controls** (no buttons to move between the album's posts), while the same album post opened from the user's profile grid shows the controls. Cause pointer: `FullScreenFeed.tsx` reads album context (`albumIds`/`albumSource`) from `store/album` (`useAlbumStore`), which is only populated when a post is opened from a grid (`PostTile.handlePress`); the home feed mounts cards with no album context, so the pager never appears. Sources: `hooks/useAlbumPosts.ts`, `hooks/useUserContextFeed.ts`. Fix direction: derive album membership from the post itself on the home feed (album id + siblings) so the controls render everywhere.
- [ ] 🔴 **CRITICAL (Kevin, 2026-09-04): "Just viewed" anchor on a profile is flaky and rarely works.** Tapping a username from a post you were viewing opens `user/[userId]?viewedPost=<id>`; when that post is deeper than the grid's first page (PAGE_SIZE=18) it isn't shown, only a "Just viewed" badge that should jump to it — and the jump usually fails. Cause pointer: `components/PostGrid.tsx` (`highlightPostId` → `effectiveHighlightId`, `jumpPending` deep-jump bulk-loads pages inside a bounded retry window and then "leaves the badge" silently when the post hasn't landed; URL-param highlights deliberately don't auto-anchor). Fix direction: resolve the post's page offset server-side (rank of the post within the user's posts) and load straight to it, or paginate until found with no retry cap + a visible loading state.
