...
import { MAX_SOURCE_BYTES, PersistError, persist, validateRequest } from './persist.ts';
...
Deno.serve({ port: PORT }, async (req) => {
  ...
  if (!presented || !timingSafeEqual(presented, expectedToken)) {
    console.warn(
      `[${SERVICE}] 401 path=${url.pathname} presented=${presented ? `len${presented.length}` : 'none'}`
    );
  ...
  if (url.pathname === '/persist') {
    ...
    if (declared > MAX_BODY_BYTES)
      return json(413, { error: `body is ${declared} bytes`, code: 'too_large' });
    const t0 = Date.now();
    let traceId = '-';
    try {
      const text = await req.text();
      if (text.length > MAX_BODY_BYTES)
        return json(413, { error: `body is ${text.length} bytes`, code: 'too_large' });
      let body: unknown;
      try {
      ...
      console.error(
        `[${SERVICE}] persist 500 trace=${traceId} after ${Date.now() - t0}ms: ${(e as Error).message}`
      );
