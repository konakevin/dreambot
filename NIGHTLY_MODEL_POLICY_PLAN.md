...
- **2026-09-12 — Kevin's rows v2 (mig 501, weighted):** every cast/scene surface `primary_models = [flux-1.1-pro, gemini-2-image, grok-imagine-image]` with `primary_weights = [50, 25, 25]` ("50 % 1.1pro for all renders and then 25/25 between grok and gemini"); fallbacks `[gemini, grok, flux-2-pro]` weighted `[45, 45, 10]`; `solo_rebuild` unchanged (flex). `resolveModel` rolls weighted (equal weights when the array is missing/short; a banned model drops out with its weight). The LOOK is then rolled from `nightly_look_approvals` for the winning model × surface (NIGHTLY_LOOKS_REFACTOR_PLAN.md §2). Because legacy always lands on 1.1-pro for couples, the shadow stamps will read `diff` for the gemini/grok share by design — Phase 3's gate is a QA night with `mode = on`, not a clean shadow.
