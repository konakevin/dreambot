...
function deriveFlavor(className: string, bucket: VotingBucket): string {
  ...
  const map: Record<string, Partial<Record<VotingBucket, string>>> = {
    ...
    'The Crowd Pleaser': {
      'lawful-rad':
        'Votes generously, posts quality, never starts drama. Probably bakes for coworkers too.',
    ...
    'The Provocateur': {
      'lawful-rad':
        'Posts things that start arguments, then votes generously anyway. Confusing but iconic.',
    ...
    'The Wanderer': {
      'lawful-rad':
        'Posts in every category, votes generously, commits to nothing. A generous free spirit.',
    ...
    'The Chaos Agent': {
      ...
      'true-neutral':
        "Posts whatever. Votes whatever. Nobody knows what's happening, including them.",
    ...
    'The Grinder': {
      ...
      'neutral-bad':
        'Posts everything. Votes with a grudge. Has a lot to say and all the time to say it.',
    ...
    'The Regular': {
      'lawful-rad':
        'Posts consistently, votes generously. The backbone of this app. Underappreciated.',
  ...
  return (
    map[className]?.[flavorBucket] ??
    'A mystery wrapped in an enigma wrapped in a questionable post.'
  );
}
...
export const STAT_DISPLAY: Record<
  ...
  { label: string; subtitle: string; tiers: string[] }
> = {
  taste: {
    label: 'TASTE',
  ...
  edge: {
    ...
    tiers: [
      ...
      'Certified crowd-divider.',
      'Causes arguments on purpose.',
    ],
  },
  range: {
    label: 'VARIETY',
