...
| female-explorer | EXPLORER (created + REVERTED) | ⚠️ MIGRATED THEN REVERTED 2026-05-12 — EXPLORER archetype + template created and path-file converted, but R1-R2 renders regressed vs legacy quality. Kevin: "this path was better before the new axes". Path reverted to legacy implementation. EXPLORER archetype + template + composer characterDnaAxes plumbing remain in lib for future investigation (likely brief language drift: legacy uses she/her consistently for Flux specificity; template went gender-neutral). | Phase 3 #7 REVERTED |
