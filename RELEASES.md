...
|-----|---------|-------|--------|---------|-----------|-------|
| `v1.0.5` | 1.0.5 | 17 | `4e4a4562cd` | 2026-07-10 | Uploaded (processing) | Build 17 resubmit (build 16 review cancelled) — adds: never route an established user into onboarding on a transient `has_ai_recipe` read failure. Plus everything from build 16: fatal Create-screen crash fix (KeyboardSwipeDismiss gesture off the worklet path), failed-dream retry now reopens in Create prefilled, multi-image album posts + unified posting flow + bulk album edit (unsave/unrepost/private/delete), per-dream notifications with an accurate badge count, follow-request accept/deny on the requester's profile, Search/Explore open the exact tapped photo, couple face-swap + friendlier dream-failed copy, startup-logo size-pop fix, security/reliability hardening (audit). |
