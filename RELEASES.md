...
|-----|---------|-------|--------|---------|-----------|-------|
| `v1.0.9` | 1.0.9 | 32 | `38eacb283e` | 2026-07-20 | Uploaded (processing) | First LOCAL build (`eas build --local`) — EAS free-plan cloud quota was exhausted. Fixes: Create-screen freeze after selecting a style; new dreams now appear in the profile/album live (no restart); double-tap heart lights instantly; Edit description on your own posts + "Share again" re-publish label; likes-sheet swipe-to-dismiss; DB connection-leak instrumentation. Build infra: fastlane installed for local builds, FB env vars moved secret→sensitive + dropped from eas.json so `$FACEBOOK_APP_ID` resolves on local builds (builds 25-31 were failed/rejected attempts; 32 is the accepted one). |
