...
|-----|---------|-------|--------|---------|-----------|-------|
| `v1.0.9` | 1.0.9 | 33 | `161aed2436` | 2026-07-20 | Uploaded (processing) | LOCAL build (`eas build --local`; EAS free-plan cloud quota exhausted). Build 33 supersedes the earlier build 32 (`38eacb283e`, same version) — adds @mention notifications for post captions (`post_mention`) + polished inbox copy. Everything else from build 32: Create-screen freeze fix; new dreams appear in profile/album live (no restart); double-tap heart lights instantly; Edit description + "Share again" re-publish label; likes-sheet swipe-to-dismiss; DB connection-leak instrumentation. Build infra fixed en route: fastlane for local builds, FB env vars secret→sensitive + dropped from eas.json so `$FACEBOOK_APP_ID` resolves locally. |
