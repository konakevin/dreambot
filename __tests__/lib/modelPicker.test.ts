...
describe('pickModel', () => {
  ...
  it('returns default model for keyword prompts without medium (no keyword SDXL fallback)', async () => {
    ...
    expect(anime.model).toBe('black-forest-labs/flux-1.1-pro');
    expect(pixel.model).toBe('black-forest-labs/flux-1.1-pro');
    expect(eightBit.model).toBe('black-forest-labs/flux-1.1-pro');
  });

  it('returns flux-1.1-pro default when no medium and normal prompt', async () => {
    const result = await pickModel('text-to-image', 'a serene mountain landscape');
    expect(result.model).toBe('black-forest-labs/flux-1.1-pro');
    expect(result.inputOverrides).toEqual({});
  });

  it('returns flux-1.1-pro default when medium provided but not in SDXL_ALWAYS and DB cache empty', async () => {
