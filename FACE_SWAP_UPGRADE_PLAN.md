...
> **PROGRAM CLOSED 2026-07-09 (owner: "done with this current face swap upgrade task").**
> Every stage resolved: shipped (0,1,2,3,5a,5b*,5c,8a-8d), rejected on evidence (4,6,7).
> *5b shipped via ACTION_POSE_EXPANSION_PLAN.md (biome pose pools + active scenarios,
> live at 25%/15% with the shuffle-bag). Standing: weekly faceswap-baseline.js;
> identity/reject telemetry on every swap; all knobs in engine_config + secrets.
...
| 4 | dual engine: Fly → easel (canary %) | never | — | ❌ REJECTED 2026-07-08 by owner, on LATENCY: easel on fal is queue-based, 1-3+ min per swap (vs ~20-30s on our Fly engine) — Kevin: "that's a non-starter for me, we can't wait that long." A multi-minute swap leg can't fit the 140s interactive render deadline. Do NOT re-propose easel/fal-queue-shaped engines for the interactive path; only a sub-30s-p95 swap API is worth benching again. The bench grid (scripts/bench-dual-engines.js) still stands as the Stage-5b contact-pose evidence on OUR engine; easel columns are reference-only |
...
| 5c | expanded single compositions (pct) | 2026-07-09, LIVE at 20% | none (owner fail-forward) | ✅ SHIPPED: three_quarter + enviro_wide presets (nightly-only, Create untouched per treads-lightly); 6/6 bench renders passed the identity gate first-take (sims 0.54-0.71); owner: "they look good". force_solo_comp bench hook; rollback = pct 0 |
