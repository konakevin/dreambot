...
| 5 | ultra / 4 MP is a model choice. Delete the legacy in-isolate dual split. | **DONE 2026-09-17.** `dualFaceSwap` + `cropRegion`/`stitchHalves`/`resizeBilinear` deleted from `_shared/faceSwap.ts` (849 → 595 lines); the in-Supabase `face-swap-dual` function (its only host) deleted + undeployed; `dualSwapDispatch.ts` is Fly-only (no URL → `dual_swap_error` → solo rebuild); `DUAL_SWAP_FANOUT` unset. Tripwire pin for faceSwap.ts 9 → 3. |
