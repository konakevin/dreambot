/**
 * AlphaBot archetype templates — the non-robot paths moved from DreamBot (2026-07-07 split, ALPHABOT.md).
 *
 * Dormant CHIBIBOT_* templates removed (cross-bot registry rejects duplicate
 * names; ChibiBot still owns them). Names keep the DREAMBOT_ prefix — the moved path files reference them and the registry is name-global.
 *
...
${
  prop
    ? `She has ${prop} — a fun retro detail worked naturally into her hands or the scene.
`
    : ''
}
...
${
  prop
    ? `He has ${prop} — a fun retro detail worked naturally into his hands or the scene.
`
    : ''
}
