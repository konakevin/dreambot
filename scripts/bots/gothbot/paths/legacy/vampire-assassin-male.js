...
module.exports = ({ sharedDNA, vibeDirective, picker }) => {
  ...
  // R6 2026-05-22 — replace solo-motion action pool with interactive HUNT-IN-ACTION pool.
  // ASSASSIN_ADVENTURE_ACTIONS had verbs like "mid-stride" / "mid-vault" that read as
  // solo poses. VAMPIRE_HUNT_INTERACTION puts a TARGET / SPOOR / hunt beat in the frame.
  const action = picker.pickWithRecency(
  ...
  const targetOrSpoor =
    pools.VAMPIRE_TARGET_OR_SPOOR && pools.VAMPIRE_TARGET_OR_SPOOR.length && Math.random() < 0.4
      ? picker.pickWithRecency(pools.VAMPIRE_TARGET_OR_SPOOR, 'vam_target_or_spoor')
      : null;
...
🚫 ABSOLUTE NO-SHIRT BAN — his chest is ALWAYS fully covered by heavy armor / heavy longcoat / chainmail / studded leather / robes / cassock. NEVER bare chest, NEVER shirtless, NEVER open-coat-revealing-pecs, NEVER ripped-shirt-with-abs-visible, NEVER unbuttoned-down-to-navel. He is a field-worn HUNTER not a fantasy-cover beefcake. If the outfit description below could be interpreted as showing chest, OVERRIDE — render him in a buttoned-up coat / closed armor / full chainmail / cassock.
...
━━━ HUNT-IN-ACTION (he is INTERACTING with the scene — target / spoor / hunt-beat visible or implied) ━━━
${action}
