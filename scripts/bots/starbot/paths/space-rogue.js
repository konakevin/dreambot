 ...
 *   lineage      ★★ THE HEADLINE — who she is physically. Alien races and striking humans, skin,
 *                unmistakable features, eyes, hair, all in ONE pool so a crest never fights a
 *                "long blonde hair" roll from a separate axis.
 *   role         title + demeanour, zero clothing (the SteamBot homogenization triad).
 *   costume      ★ cut, material, sheen, one accessory. Sexy through silhouette, never body words.
 *   hero_object  the thing in her hand. Nothing with a screen or a label.
...
// Lineage entries open "A Vessari woman with …" — keep the name, lower-case the article so the
// costume can be spliced in right after the name: "a Vessari woman wearing …, with …".
function splitLineage(entry) {
  const m = String(entry).match(/^A\s+(.+?\swoman)\s+with\s+(.+)$/i);
  if (m) return { who: m[1], traits: m[2].replace(/\.\s*$/, '') };
  return { who: 'woman', traits: String(entry).replace(/\.\s*$/, '') };
}
...
module.exports = ({ vibeDirective, picker }) => {
  ...
  const costume = picker.pickWithRecency(need(COSTUME, 'costume'), 'space_rogue_costume');
  ...
  return `You are a production designer and costume designer writing ONE painted science-fiction character still for StarBot: a sleek, confident woman in a place as designed and as cool as she is. Painterly sci-fi concept art, one dramatic light source, a committed two-colour story.
...
RULE 2 — SHE IS DRESSED FIRST, THEN DESCRIBED. The very first thing written about her is her costume, exactly as given, fully covering her exactly as the costume says. Only then her skin, features, eyes and hair, all rendered unmistakably. Her face is fully visible. She is "she" and "her" throughout, confident and in command of the room.
RULE 3 — ONLY WHAT IS NAMED IS IN THE FRAME. One venue, one woman, at most one other presence, room around her. Every flat surface of the place is dressed with objects or texture and every surface is plain of writing.

★ VENUE (opens the picture): ${venue}
★ COLOUR STORY (owns the palette): ${light}
AIR: ${air}
★ SHE IS: a ${lineage.who} — ${lineage.traits}
★ SHE WEARS: ${costume}
BEAT: ${beat}
ROLE: ${role}
...
[a ${lineage.who} wearing (her costume exactly as given), with (her skin, features, eyes and hair), her face fully visible, (her beat) in this place, her whole figure in frame],
