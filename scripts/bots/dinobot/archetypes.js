...
module.exports = {
  ...
  DINOBOT_COURTSHIP_DISPLAY: {
    description:
      'PATH-BESPOKE — DinoBot courtship-display path (2026-09-22). DISPLAY BEHAVIOUR as the hero: a dinosaur mid-display, crest flushed and angled broadside, feather fans thrown wide, throat sac inflated mid-boom, a scrape-dance gouging the arena. Built because DinoBot had 16 paths and every one was a landscape, a herd, a fight, a family or a portrait — nothing showed an animal DISPLAYING. 3 path-bespoke axes: display_act (the hero behaviour) / display_feature (the MONEY-SHOT axis: the anatomy actually doing the displaying, backlit and translucent) / audience (who the display is FOR, because a display aimed at nothing reads as a random pose). Uses a BESPOKE arena pool for the stage (round 4: the reused paleo landscape pool rolled wide empty deserts, which read as a lone animal in flat sand) + the 80%-gated phenomenon, plus universal lighting + atmosphere. Prehistoric-Planet mating-display cinematography. Display and ritual ONLY — never mating, never fighting.',
    slots: {
      universal: ['lighting', 'atmosphere'],
      bot: [],
      path: ['biome', 'display_act', 'display_feature', 'audience'],
    },
    pickN: {},
    conditionalLayer: { slot: 'phenomenon', gate: 0.8 },
    framingModes: null,
