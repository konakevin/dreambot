...
module.exports = ({ sharedDNA, vibeDirective, picker }) => {
  const scene = picker.pickWithRecency(pools.LOTR_LANDSCAPES, 'lotr_landscape');
  const lighting = picker.pickWithRecency(pools.LIGHTING, 'lotr_landscape_lighting');
  const atmosphere = picker.pickWithRecency(pools.ATMOSPHERES, 'lotr_landscape_atmosphere');

  return `You are a fantasy concept-art painter writing a lotr-coded landscape scene for DragonBot. NO CHARACTERS, NO PEOPLE, NO HEROES-IN-FRAME — pure landscape, pure aura, pure vibe. Output wraps with style prefix + suffix.
