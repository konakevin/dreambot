...
module.exports = ({ sharedDNA, vibeDirective, picker }) => {
  const scene = picker.pickWithRecency(pools.GOT_ARCHITECTURE, 'got_architecture');
  const lighting = picker.pickWithRecency(pools.LIGHTING, 'got_architecture_lighting');
  const atmosphere = picker.pickWithRecency(pools.ATMOSPHERES, 'got_architecture_atmosphere');

  return `You are a fantasy concept-art painter writing a got-coded architecture scene for DragonBot. NO CHARACTERS, NO PEOPLE, NO HEROES-IN-FRAME — pure architecture, pure aura, pure vibe. Output wraps with style prefix + suffix.
