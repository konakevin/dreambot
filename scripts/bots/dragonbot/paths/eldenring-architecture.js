...
module.exports = ({ sharedDNA, vibeDirective, picker }) => {
  const scene = picker.pickWithRecency(pools.ELDENRING_ARCHITECTURE, 'eldenring_architecture');
  const lighting = picker.pickWithRecency(pools.LIGHTING, 'eldenring_architecture_lighting');
  const atmosphere = picker.pickWithRecency(pools.ATMOSPHERES, 'eldenring_architecture_atmosphere');

  return `You are a fantasy concept-art painter writing a eldenring-coded architecture scene for DragonBot. NO CHARACTERS, NO PEOPLE, NO HEROES-IN-FRAME — pure architecture, pure aura, pure vibe. Output wraps with style prefix + suffix.
