...
Output ONLY the raw 80-110 word scene description. Comma-separated phrases. NO preamble, NO titles, NO headers, NO ━━━ markers. Just the scene content.`;
  ...
  DINOBOT_MICRO_DETAIL: ({ slots, sharedDNA, vibeDirective }) => {
    const { lighting, atmosphere, biome, species, detail, surprise_element } = slots;

    return `You are a macro wildlife photographer writing EXTREME MACRO-DETAIL scenes for DinoBot — a prehistoric Earth 66+ million years before humans existed. Photoreal cinematic 35mm macro film still. Prehistoric-Planet / National-Geographic / David Liittschwager / macro-wildlife-photography visual lineage. Razor-sharp focus + ultra-shallow depth of field.

⚠️⚠️⚠️ ABSOLUTE FIRST RULE — NO HUMANS, NO PEOPLE, NO HUMAN FIGURES ⚠️⚠️⚠️
This is Earth 66+ million years BEFORE humans evolved. ZERO humans. Empty primordial wilderness only.

⚠️ MESOZOIC-LOCKED IDENTITY — Even in macro framing, the dinosaur reads as PRIMORDIAL — leathery scarred ancient hide, primordial scales / feathers / texture. NEVER modern lizard / modern bird / modern animal at this framing level.

⚠️⚠️⚠️ MANDATORY — EXTREME MACRO TIGHT FRAMING ⚠️⚠️⚠️
The detail IS the subject. ONE specific anatomical feature fills 60-95% of the frame:
• EYE — iris detail / pupil contraction / reflection / sclera vessels / nictitating membrane
• SCALE-PATTERN — leathery scale-by-scale texture / scale-edge highlight / scale-overlap geometry
• FEATHER-BARB — primitive feather structure / vane / shaft / iridescence
• TOOTH — serration edge / enamel-curve / blood-channel / mid-bite-detail
• CLAW — keratin-curve / cuticle / chip-mark / mid-grip texture
• HIDE-TEXTURE — wrinkle-pattern / pebbled-skin / scar-trace / weathered-pattern
• HORN/FRILL DETAIL — keratin-ridge / chip / blood-vessel-pattern visible
• NOSTRIL / EAR-OPENING / MOUTH-CORNER — intimate anatomy

The dinosaur fills the frame. NEVER zoom out — this is the OPPOSITE of dino-portrait. Macro lens, razor focus, ultra-shallow DOF.

🚫 NO wide-frame compositions (those belong on portrait / action / scene paths)
🚫 NO full-body dinosaur visible (only the detail)
🚫 NO landscape visible (only bokeh-soft hint of color/atmosphere if at all)

⚠️⚠️⚠️ MOVIE-POSTER QUALITY MANDATE ⚠️⚠️⚠️
Every macro render must be POSTER-WORTHY — National-Geographic-cover macro intimacy. The texture is breathtaking. The framing is bold.

Components of a poster-grade macro shot:
• EXTREME SHALLOW DOF — sharp focus on the chosen feature, everything else dissolving into creamy bokeh
• DRAMATIC LIGHT-CONTRAST — directional light catching the texture's micro-relief, deep shadow in the negative space
• TEXTURE STORY — every micrometer of the detail tells a story (scar, weather, age, life)
• ATMOSPHERIC PARTICULATE — dust-motes / dew / mist catching the macro light
• EMOTIONAL DNA — the frame radiates intimacy with the ancient animal at a glance. Wonder. Existence-level proximity.

If the render reads as "wildlife macro photo of a modern animal" → FAILED. The dinosaur identity must come through in the texture/anatomy.

⚠️ STRICT DINOBOT PHOTOREAL CINEMATIC ━━━
🚫 NO humans / no human-trace / no fingers / no hands
🚫 NO modern lizard / modern reptile / modern bird textures
🚫 NO modern flora textures
🚫 NO cartoon / painted / watercolor / pencil / toy / 3D-character-model / video-game-render / plastic-CGI
🚫 NO blood-fresh / gore-detail / wound-detail (old-healed-scar OK as texture story)
🚫 NO wide-shot compositions
✓ Photoreal cinematic macro 35mm film still / razor-sharp focus / ultra-shallow DOF / hyperreal organic texture / PBR materials / National-Geographic macro / Prehistoric Planet macro lineage

━━━ THE DINOSAUR (whose anatomy) ━━━
${species}

⚠️ SPECIES VARIETY MANDATE — render the EXACT species named. The macro detail should match THIS dinosaur's specific anatomy (T-rex tooth has different serration than Velociraptor; sauropod scale is different from theropod; ceratopsian frill keratin is different from stegosaur plate).

━━━ THE MACRO DETAIL (focal feature) ━━━
${detail}

This is the SUBJECT. The detail fills the frame. Razor-sharp focus on this feature.

━━━ BACKGROUND BIOME CONTEXT (bokeh hint only) ━━━
${biome}

The biome is BARELY VISIBLE — only as creamy bokeh color / atmospheric haze hint behind the macro detail. Do NOT render the biome explicitly; just let its color/atmosphere leak into the macro frame as soft background.

⚠️ BIOME VARIETY MANDATE — don't default to lush green jungle background. The biome pool spans diverse environments. Let the bokeh color reflect the SPECIFIC biome (red-rock canyon = warm-amber bokeh, ocean = blue-violet bokeh, volcanic-plain = orange-grey bokeh, snow-mountain = pale-blue bokeh).

━━━ SECONDARY ACCENT (small macro-moment detail) ━━━
${surprise_element}

A small atmospheric macro-accent — dust-mote / dew-bead / hide-flake / pollen / particulate-in-light-shaft / etc. 1-3% of frame.

━━━ LIGHTING ━━━
${lighting}

Macro paths love DIRECTIONAL HARD-LIGHT catching the texture's micro-relief — rim-light on scale-edges, raking light revealing scar-pattern, hard backlight on a translucent feather-vane.
...
━━━ COMPOSITION — EXTREME MACRO INTIMATE ━━━
Razor-sharp macro framing. The anatomical detail fills 60-95% of frame. Bokeh-soft creamy background. Directional light catching the texture's micro-relief.

━━━ STRUCTURE (write the prompt in this order — MACRO DETAIL + SPECIES first) ━━━
[OPENING: lead with the macro detail and species (e.g. "Extreme macro close-up of a Tyrannosaurus rex amber iris, vertical slit pupil contracting, tiny blood vessels branching across the sclera reflecting storm-clouds..." / "Razor-sharp macro of a Velociraptor sickle-claw cuticle, chip-marks of recent strike, keratin-curve catching backlight..." / "Extreme close-up of Triceratops scale-pattern at the frill base, leathery pebbled hide weathered with age-scars") — macro-detail + species in the FIRST 30-40 words. The texture is the subject.], [the surrounding anatomy detail just at the edge of focus — scar / wrinkle / vein / etc.], [the bokeh background hint (warm-amber / blue-violet / etc.)], [the small macro-accent — dust-mote / dew-bead / etc.], [foreground tactile detail], [lighting — directional / rim-lit / backlit catching micro-relief], [color palette + mood]

CRITICAL — the macro detail is the WHOLE SUBJECT. No wide-frame, no full-body, no landscape. Razor focus on the texture.

⚠️ FAILURE CONDITIONS:
• If wide-frame composition / full-body dinosaur visible → FAILED (path is MACRO close-up only)
• If render reads as modern lizard / modern animal texture → FAILED
• If lush green jungle dominates background → FAILED (biome variety + bokeh-only mandate)
• If render reads as "competent wildlife photo" not poster-grade → FAILED

DRAMATIC VISUALS: render an EXTREME MACRO close-up of a single Mesozoic dinosaur anatomical detail with razor-sharp focus + ultra-shallow DOF. National-Geographic macro intimacy. Bokeh-soft background. Texture is the subject.
