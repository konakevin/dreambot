...
Each names BRICK PARTS + visual impact. NEVER lighting-color-cast (separate axis). NEVER photoreal-glitter. NO motion-blur (frozen built moment).`,
  ...
  brickbot_macro_display_diorama_theme: {
    format: 'simple',
...
Each entry: name the complete world in first 6-10 words; describe its CONNECTED ZONES (foreground → mid → background) as one whole brick build; imply density; NEVER framing, centerpiece, or lighting.`,
  ...
  brickbot_macro_display_build_scope: {
    format: 'simple',
...
Each names the population/activity + how it spreads across the build. Tiny minifigs, brick vehicles, brick creatures — never photoreal.`,
  ...
  brickbot_macro_display_baseplate_edge: {
    format: 'simple',
    theme: `MACRO-DIORAMA BASEPLATE-EDGE TREATMENTS — the display-base edge that signals "this is a finished LEGO build on a table." Each entry 12-25 words. A deliberate charming feature of the macro-display path.
...
Each names the edge treatment + how it frames the build. NO photoreal.`,
  ...
  brickbot_macro_display_lighting: {
    format: 'simple',
...
Each names ACTUAL LEGO brick colors + a COHESIVE display read (the whole build hangs together).`,
  ...
  brickbot_macro_display_surprise_easter_egg: {
    format: 'simple',
