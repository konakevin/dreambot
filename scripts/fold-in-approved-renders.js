 ...
 * is deliberately left alone: it is the real render time and worth keeping honest.
 *
 * Usage:
...
const APPROVED_PATHS = [
  'balloon-festival', 'airfield-biplanes', 'undergrowth-scale', 'den-and-burrow',
  'courtship-display', 'snowline-forest', 'desert-dunes', 'amber-forest',
  'mushroom-apothecary', 'star-charting', 'castle-town-gate', 'floating-market-canal',
  'volcano-forge', 'game-center-arcade', 'cozy-farming-life-sim', 'brass-glasshouse',
  'snow-globe-world', 'acorn-boat-regatta',
];

/** Tripwire: these were deleted. None may appear here. */
const CUT_PATHS = [
  'alpine-wildflower-meadow', 'orchid-cloud-forest', 'coastal-cliff-bloom',
  'sheep-shearing-day', 'apiary-beekeeping', 'hay-baling-summer', 'lambing-season',
  'puppet-theatre', 'sand-toy-beachworks', 'bath-toy-flotilla', 'honey-harvest',
  'autumn-seed-gathering', 'observatory-tower', 'ice-cavern', 'archaeology-dig',
  'tidal-flat-tracks', 'onsen-evening', 'rooftop-telegraph',
];
...
function finalBatch(rows) {
  ...
  for (const r of sorted) {
    ...
    if (prev !== null && t - prev > GAP_MIN * 60000) { batches.push(cur); cur = []; }
    cur.push(r); prev = t;
  }
  if (cur.length) batches.push(cur);
  const substantial = batches.filter((b) => b.length >= MIN_BATCH);
  return substantial.length ? substantial[substantial.length - 1] : (batches[batches.length - 1] || []);
}
...
async function main() {
  ...
  for (const [id, name] of botName) {
    const { data: first } = await sb.from('uploads').select('posted_at').eq('user_id', id)
      .eq('is_public', true).not('posted_at', 'is', null).order('posted_at', { ascending: true }).limit(1);
    const { data: last } = await sb.from('uploads').select('posted_at').eq('user_id', id)
      .eq('is_public', true).not('posted_at', 'is', null).order('posted_at', { ascending: false }).limit(1);
  ...
  for (const p of APPROVED_PATHS) {
    if (HOLD_PATHS.includes(p)) { console.log(`   ⏸  ${p}: HELD (pending the move to TinyBot)`); continue; }
    const rows = [];
    for (let from = 0; ; from += 1000) {
      const { data, error } = await sb.from('uploads')
        .select('id, image_url, created_at, is_public, is_posted, posted_at, user_id, recipe')
        .eq('recipe->>path', p).order('created_at', { ascending: true }).range(from, from + 999);
      if (error) die(`query failed for ${p}: ${error.message}`);
      rows.push(...data);
      if (data.length < 1000) break;
    }
    if (!rows.length) { console.log(`   ⚠️ ${p}: no renders found, skipping`); continue; }
    ...
    if (PINNED_BATCH[p]) {
      ...
      if (chosen.length !== PINNED_BATCH[p].length)
        die(`${p}: pinned batch expects ${PINNED_BATCH[p].length} renders, matched ${chosen.length}`);
    } else {
    ...
    for (const r of chosen) {
      ...
      if (!s) { console.log(`   ⚠️ ${botName.get(r.user_id)} has no public history to scatter into; skipping ${r.id}`); continue; }
  ...
  for (const b of Object.keys(byBot).sort()) console.log(`   ${b.padEnd(12)}${String(byBot[b].length).padStart(3)} renders`);
  ...
  if (!execute) {
    ...
    for (const x of plan.slice(0, 4)) console.log(`     e.g. ${x.bot}/${x.path} → ${x.when.slice(0, 10)}`);
    return;
  }
  ...
  let n = 0;
  for (const x of plan) {
    const { error } = await sb.from('uploads')
      .update({ is_posted: true, is_public: true, posted_at: x.when })
