...
const MEDIUMS = [
  ...
  {
    key: 'anime',
    flux: 'Anime illustration, clean ink linework, cel-shaded coloring, expressive detailed eyes, Makoto Shinkai inspired backgrounds, vibrant saturated colors',
  },
  {
    key: 'lego',
    flux: 'LEGO brick diorama, everything constructed from LEGO pieces, plastic studs visible on every surface, minifigure characters, soft realistic lighting',
  },
  {
    key: 'ghibli',
    flux: 'Studio Ghibli animation style, soft painterly rendering, warm natural color palette, detailed painted backgrounds, Miyazaki quality',
  },
  {
    key: 'claymation',
    flux: 'Claymation stop-motion animation, smooth sculpted clay characters, visible fingerprint textures, glass bead eyes, handcrafted miniature sets',
  },
  {
    key: 'disney',
...
const VIBES = [
  {
    key: 'cinematic',
    dir: 'Frame from an Oscar-winning film. Teal-and-orange grade, motivated lighting, frozen narrative tension.',
  },
  { key: 'dreamy', dir: 'Soft ethereal haze, omnidirectional glow, pastels, weightlessness.' },
  {
    key: 'epic',
    dir: 'SCALE. Impossibly vast — towering mountains, endless skies, cosmic expanses. The viewer feels small and awed.',
  },
  {
    key: 'dark',
    dir: 'Deep shadow, velvety blacks. Light is rare and precious. Caravaggio chiaroscuro.',
  },
  {
    key: 'whimsical',
    dir: 'Physics optional. Wrong sizes, candy-bright colors, floating islands, playful gravity.',
  },
  {
    key: 'psychedelic',
    dir: 'Reality melting, fractals, maximum saturation, patterns at every scale.',
  },
];
...
async function generateKontext(prompt, inputImage, sceneImage) {
  ...
  const res = await fetch(
    'https://api.replicate.com/v1/models/black-forest-labs/flux-kontext-pro/predictions',
    {
      ...
      headers: { Authorization: `Bearer ${REPLICATE_TOKEN}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        input: { prompt, input_image: sceneImage, aspect_ratio: '9:16', output_format: 'jpg' },
      }),
    }
  );
...
async function main() {
  ...
  const cast = (profile?.dream_cast ?? []).filter(
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    console.log(
      `[${i + 1}/${COUNT}] ${castMember.role} | ${medium.key} | ${vibe.key} | ${category}`
    );
