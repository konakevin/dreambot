...
function generateUsername(index) {
  ...
  return STYLES[index % STYLES.length](p, s, n)
    .toLowerCase()
    .replace(/[^a-z0-9_.]/g, '');
}
...
const CATEGORIES = [
  ...
  },
  {
    key: 'food',
    queries: [
      'ramen steam close up',
      'pizza cheese pull',
      'acai bowl tropical',
      'sushi aesthetic',
      'ice cream melting colorful',
      'street tacos night',
    ],
  },
  {
  ...
  },
  {
    ...
    queries: [
      'dj neon festival',
      'crowd surfing concert',
      'guitar campfire sunset',
  ...
  },
  {
    key: 'art',
    queries: [
      'neon sign city night',
      'graffiti mural colorful',
      'ceramic pottery art',
  ...
  },
  {
    ...
    queries: [
      ...
      'perfume bottle elegant',
      'flowers aesthetic flat lay',
    ],
  },
...
const CAPTIONS = {
  people: [
    'caught this one by accident lol',
    'idk why this pic goes so hard',
    'me pretending im a model',
    'the sun was hitting different today',
    'ok but look at this lighting??',
    'no im not ok',
    'fit check nobody asked for',
    'tuesday energy',
    'living my best life or whatever',
    'someone come get their mans',
    'just vibes honestly',
    'this is my good side apparently',
    'pov: you actually left the house',
    'my therapist would be proud',
    'main character moment fr',
    'im never taking a pic this good again',
    'not pictured: the 47 bad takes before this',
  ],
  animals: [
    'tell me this isnt the cutest thing ever',
    'he has no idea how famous hes about to be',
    'would literally die for this animal',
    'look at this dumdum',
    'shes judging you rn',
    'rent free in my head since forever',
    'the face when you hear the treat bag',
    'hes doing his best ok',
    'chaotic energy',
    'this ones got the whole neighborhood fooled',
    'adopted him 3 years ago best decision ever',
    'proof that dogs are better than people',
    'she literally posed for this',
    'how is this real',
    'the audacity of this cat',
  ],
  food: [
    'ate this 2 hours ago still thinking about it',
    'is it normal to photograph your food this much',
    'the first bite was spiritual',
    'found this spot on accident and wow',
    'brunch was a personality trait today',
    'i made this and honestly shocked',
    'if youre not eating this what are you doing',
    'this taco changed my life not even joking',
    'calories dont count on vacation right',
    'my kitchen my rules',
    'street food > fancy restaurants every time',
    'the cheese pull tho',
    'idk who needs to hear this but go eat something good today',
    'im not a chef but',
  ],
  nature: [
    'phone doesnt do it justice tbh',
    'i just stood here for like 20 minutes',
    'ok earth you didnt have to go that hard',
    'sometimes you just gotta stop and look up',
    'no filter i swear',
    'woke up at 5am for this and zero regrets',
    'places like this actually exist??',
    'this is why i hike',
    'the clouds were showing off today',
    'im moving here bye',
    'peaceful af',
    'how does this not have more likes come on',
    'currently offline from life',
    'put the phone down and just breathe',
    'the universe said here you go',
  ],
  funny: [
    'im crying actual tears',
    'why is this so accurate',
    'showed this to everyone at work',
    'me at 3am for no reason',
    'the way i just screamed',
    'im sending this to everyone i know',
    'this should not be this funny',
    'pov me trying to adult',
    'how do i unsee this',
    'i have watched this 400 times',
    'whoever made this deserves an award',
    'my sense of humor is broken and i dont care',
  ...
  music: [
    ...
    'the bass hit my chest and i felt alive',
    'best night of my life no cap',
    'if you know you know',
    'this set was unreal',
    'my ears are still ringing worth it',
    'music is literally therapy',
    'the crowd energy was insane',
    'vinyl sounds hit different',
    'found this artist last week and im obsessed',
    'the drop tho',
  ...
  sports: [
    ...
    'been rewatching this nonstop',
    'clean af',
    'this is what happens when you dont give up',
    'physics said no and they said watch me',
    'the commitment is insane',
    'my jaw literally dropped',
    'years of practice in one clip',
    'thats going on the highlight reel',
    'someone call the police this was a robbery',
    'im just a spectator and im exhausted',
  ],
  art: [
    'walked past this three times before i actually looked',
    'ok this is actually incredible',
    'the detail up close is wild',
    'art that makes you feel something >>> ',
    'i want this on my wall',
    'took me a sec to realize this was painted not a photo',
    'the colors are even crazier in person',
    'support your local artists seriously',
    'i dont know art but i know what i like',
    'the texture on this',
    'someone put their whole soul into this',
    'gallery hopping is my new personality',
  ...
  beauty: [
    'the blend tho',
    'glass skin era',
    'this look took me 2 hours and worth every minute',
    'new routine and my skin is thriving',
    'the glow up is real',
    'soft glam always wins',
    'finally nailed this look',
    'skincare is self care',
    'obsessed with this shade',
    'tutorial coming soon maybe',
    'the lashes are doing the heavy lifting',
  ...
  cute: [
    ...
    'the little face omg',
    'i need twelve of these immediately',
    'cuteness overload',
    'this healed me',
  ],
};
    ...
    if (error.message.includes('already been registered')) {
      ...
      if (existing) {
        if (avatar)
          await supabase
            .from('users')
            .update({ avatar_url: avatar, username })
            .eq('id', existing.id);
      ...
      while (true) {
        ...
        if (found) {
          ...
          if (retry?.user) {
            if (avatar)
              await supabase.from('users').update({ avatar_url: avatar }).eq('id', retry.user.id);
...
async function main() {
  ...
  if (protectedIds.length > 0) {
    ...
    await supabase
      .from('post_shares')
      .delete()
      .not('sender_id', 'in', pIds)
      .not('receiver_id', 'in', pIds);
    await supabase
      .from('vote_streaks')
      .delete()
      .not('user_a', 'in', pIds)
  ...
  await supabase
    .from('streak_cron_state')
    .update({ last_processed_at: '2000-01-01T00:00:00Z' })
    .eq('id', 1);
  ...
  for (const f of friends) {
    ...
    await supabase
      .from('friendships')
      .upsert(
        { user_a: a, user_b: b, status: 'accepted', requester: f.id },
  ...
  // ── 5. Fetch photos ──────────────────────────────────────────────────────
  console.log('\n📷 Fetching Pexels photos (all queries per category)...');
  const photoPool = {};
  for (const cat of CATEGORIES) {
    ...
    photoPool[cat.key] = allPhotos.filter((p) => {
  ...
  for (const user of strangers) {
    const toFollow = shuffle(allUsers.filter((u) => u.id !== user.id)).slice(
      0,
      Math.floor(allUsers.length * (0.2 + Math.random() * 0.3))
    );
    ...
    for (let i = 0; i < friends.length; i++) {
      ...
      const vote = isMilestonePost
        ? i < 9
          ? 'rad'
          : 'bad'
        : hash(p, i) < (FRIEND_BIAS[friends[i].username] ?? 0.5)
          ? 'rad'
          : 'bad';
  ...
  const updatePromises = Object.entries(voteCounts).map(([uploadId, counts]) =>
    supabase
      .from('uploads')
      .update({
        total_votes: counts.rad + counts.bad,
        rad_votes: counts.rad,
        bad_votes: counts.bad,
      })
      .eq('id', uploadId)
  );
  ...
  for (const friend of friends) {
    ...
    const shareable = shuffle(
      allPosts.filter((p) => p.user_id !== kevin.id && p.user_id !== friend.id)
    );
  ...
  for (let i = 0; i < friends.length - 1; i++) {
    ...
    const shareable = shuffle(
      allPosts.filter((p) => p.user_id !== sender.id && p.user_id !== receiver.id)
    );
  ...
  log(
    `${shareRows.length} shares total (${kevinInbox.length} to Kevin, ${kevinUnseen.length} unseen)`
  );
  ...
  const COMMENT_TEMPLATES = [
    ...
    'this aint it',
    'genuinely cant tell if this is rad or bad',
    'respectfully no',
    'ok i see you',
    'sleeper hit',
    ...
    'im torn on this one honestly',
    'the lighting carries this hard',
    'clean',
    'mid',
    'elite',
    'whoever posted this has taste',
    'instant rad from me',
    'voting bad and i dont feel bad about it',
    'rad all day',
    'wait is this edited or real',
    'this photo has a vibe idk',
    'straight heat',
    'this pic goes crazy actually',
    'the composition tho',
    'top 5 post ive seen this week',
    'love this',
    'terrible lol',
    'vibes',
    'not my thing but respect',
    'i keep coming back to this one',
    'this shouldnt work but it does',
    'bruh who uploaded this 💀',
    'obsessed',
    'meh',
    'lowkey fire',
    'highkey fire',
    ...
    'why does this have so few votes',
    'im voting rad just for the audacity',
    'bold choice posting this',
  ];
  ...
  const commentsWithReplies = shuffle(insertedComments).slice(
    0,
    Math.floor(insertedComments.length * 0.4)
  );
  ...
  for (const friend of friends) {
    const friendComments = shuffle(insertedComments).slice(
      0,
  ...
  if (protectedIds.length > 0) {
    await supabase
      .from('notifications')
      .delete()
      .not('recipient_id', 'in', `(${protectedIds.join(',')})`);
  } else {
    await supabase.from('notifications').delete().not('id', 'is', null);
  }
  ...
  const { data: d2 } = await supabase.rpc('get_following_feed', {
    p_user_id: kevin.id,
    p_limit: 50,
  });
  ...
  console.log(
    `  ${insertedComments.length} comments + ${replyCount} replies + ${commentLikeRows.length} likes`
  );
