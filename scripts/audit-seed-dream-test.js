...
async function fetchAll(table, pools) {
  ...
  for (const pool of pools) {
    ...
    for (;;) {
      let q = sb
        .from(table)
        .select('id,pool,category,sub_theme,scene,attire')
        .eq('disabled', false)
        .eq('pool', pool);
      if (CATEGORY) q = q.eq('category', CATEGORY); // --category fall: one holiday only
...
async function run(label, rows) {
  ...
  const worker = async () => {
    while (batches.length) {
      ...
      try {
        ...
        for (const v of vs) {
          const r = b.find((x) => x.id === v.id);
          if (r) verdicts.push({ ...r, verdict: v.verdict, why: v.why });
        }
      } catch (e) {
  ...
  for (const v of verdicts) {
    const k = `${v.pool}/${v.category ?? v.sub_theme ?? '?'}`;
    byCat[k] = byCat[k] || { n: 0, plain: 0 };
    byCat[k].n++;
    if (v.verdict === 'PLAIN') byCat[k].plain++;
  }
  ...
  for (const [k, v] of Object.entries(byCat).sort(
    (a, b) => b[1].plain / b[1].n - a[1].plain / a[1].n
  ))
    if (v.plain) console.log(`  ${k.padEnd(36)} ${String(v.plain).padStart(3)} / ${v.n}`);
  return verdicts;
}

(async () => {
  if (process.argv.includes('--disable')) {
    const file = arg('disable');
    const verdicts = JSON.parse(fs.readFileSync(file, 'utf8')).filter((v) => v.verdict === 'PLAIN');
    const table = arg('table');
    if (!table) throw new Error('--table required with --disable');
    const ledger = {
    ...
    const { error } = await sb.from(table).update({ disabled: true }).in('id', ledger.ids);
    if (error) throw error;
    const lp = path.join(OUT, `disable-plain-${table}-${Date.now()}.json`);
    fs.writeFileSync(lp, JSON.stringify(ledger, null, 1));
    console.log(`disabled ${ledger.ids.length} PLAIN rows in ${table}; ledger ${lp}`);
    return;
  }
  if (process.argv.includes('--cards')) {
    const { data } = await sb
      .from('location_cards')
      .select('id,name,biome,atmosphere,cinematic_phrases')
      .not('picker_category', 'is', null);
    const rows = data.map((c) => ({
      id: c.id,
      pool: 'card',
      category: c.biome,
      scene: `${c.name}. ${c.atmosphere ?? ''} ${(c.cinematic_phrases || []).slice(0, 4).join('; ')}`,
      attire: 'contemporary travel clothes',
    }));
    await run('location_cards', rows);
    return;
  }
  ...
  console.log(
    `${table} [${pools.join(',')}]: ${rows.length} enabled rows → judging in batches of 25 (conc ${CONC})`
  );
