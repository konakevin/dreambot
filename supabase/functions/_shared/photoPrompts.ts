...
${vibe.slice(0, 200)}${hint ? `\n${hint}` : ''}`,
  ...
  photography: {
    model: 'kontext-max',
    buildPrompt: (_photo, vibe, hint) => `Make this photo look professionally shot. CRITICAL: Do NOT alter the person's face, skin, features, age, or identity in ANY way. The person must be pixel-perfect identical.

Only adjust: subtle background blur, color grading, and overall tone. Nothing else changes. The subject is untouched. Portrait 9:16.

Apply this mood ONLY through color grading and tonal adjustments:
${vibe.slice(0, 200)}${hint ? `\n${hint}` : ''}`,
  },

  vintage_film: {
    model: 'kontext-max',
    buildPrompt: (_photo, vibe, hint) => `Apply a vintage film look to this photo. CRITICAL: Do NOT alter the person's face, skin, features, age, or identity in ANY way. The person must be pixel-perfect identical.

Only add: organic film grain texture, subtle light leaks from edges, slightly lifted shadows, gentle highlight bloom. The person is completely untouched — only the film overlay changes. Portrait 9:16.

Apply this mood through film color science and light leak tones:
${vibe.slice(0, 200)}${hint ? `\n${hint}` : ''}`,
  },
