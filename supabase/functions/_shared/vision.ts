 ...
 */
/**
 * Hair the focused read must NOT overwrite. HAIR_COLOR_PROMPT answers from a flat
 * ~10-word natural palette, so letting it clobber an already-distinctive clause
 * FLATTENS the person: the 2026-09-01 fleet re-scan turned "Long purple-to-teal
 * ombre wavy hair" into "blonde hair" and "salt-and-pepper white hair" into
 * "dark brown hair with salt-and-pepper temples". Dyed / multi-tone / two-tone
 * hair is exactly the identity signal we least want averaged away, so when the
 * scanned clause already carries one of these, keep it.
 */
const DISTINCTIVE_HAIR_RE =
  /\b(ombre|balayage|dyed|bleached|highlights?|lowlights?|streaks?|two[- ]tone|multi[- ]tone|rainbow|pink|purple|violet|teal|blue|green|lavender|pastel|magenta|burgundy|salt[- ]and[- ]pepper|frosted|tipped|roots)\b/i;
 ...
 * (extractHair + the render color anchor) works on corrected data.
 *
 * Skips the swap when the existing clause is already distinctive (see above) —
 * the focused read is a correction for plain natural colours, not a licence to
 * repaint someone's dyed hair.
 */
export function replaceHairColorInSummary(summary: string, color: string): string {
  ...
  // Distinctive/dyed hair wins over the flat-palette focused read.
  if (DISTINCTIVE_HAIR_RE.test(parts[idx])) return summary;
...
export const VISION_PROMPTS = {
  ...
  castPerson:
    'Your response MUST BEGIN with a HEADER LINE and nothing before it, in EXACTLY this format: the gender ("Male" or "Female"), a comma, then the build (exactly ONE of "thin", "athletic", or "average"). Examples: "Female, average" or "Male, athletic". Commit to the GENDER and the BUILD on this first line — they are the two most important calls, so decide them FIRST. BUILD RULE (critical, honor it exactly): use ONLY thin / athletic / average. Anyone who is not clearly thin or clearly athletic is "average". NEVER output any other size or shape word anywhere (NO curvy, full-figured, soft, stocky, petite, heavy, heavyset, plus, plus-size, overweight, busty, chubby, large, big) — "average" covers everyone else, so no one is ever given a heavy or unkind label. Then describe them for an AI artist creating a flattering stylized character — focus on the FACE and HAIR. Include: exact age estimate, face shape, eye color, hair — give the LENGTH (short, medium, or long), the exact COLOR (like sandy brown or chestnut), the TYPE if it is not straight (wavy, curly, or tightly coily), and BANGS with their kind if they have them (e.g. "with soft wispy bangs", "with blunt bangs", "with curtain bangs"). Describe the HAIRCUT only — NEVER how the hair happens to be worn in this one photo: do NOT write ponytail, bun, half-up, updo, braids, pigtails, pin curls, "worn in", "pulled back", "swept back", or any part/hairline wording (no "center-parted", no "swept back from the forehead"). Those are one day of styling and would lock the person into that look forever. If they have no bangs, simply omit bangs — never write "no bangs", skin tone, any distinguishing features (glasses, freckles, jewelry, tattoos). Do NOT describe clothing or accessories that change — the art generator will dress them in the scene style. Do NOT describe body size or shape again in the prose — the header build word is the ONLY body descriptor allowed. For facial hair be EXTREMELY precise: clean-shaven, light stubble, heavy stubble, short trimmed beard, medium beard, or full long beard — do NOT exaggerate length or thickness, stubble is NOT a beard. Skip unflattering details (under-eye bags, dark circles, blemishes, wrinkles) — describe their best version. 3 sentences max. Be EXTREMELY specific — the more detail, the better the resemblance.\n\nAfter the description, add a NEW LINE starting with "AGE:" followed by your best numeric age estimate as an integer only (no words, no qualifiers, no range). Example: "AGE: 34". Estimate their TRUE age as accurately as you can — do NOT lean younger (the model already tends to under-read age, so an honest estimate lands closest to reality).\n\nAfter the AGE line, add a NEW LINE starting with "TRAITS:" followed by a single concise sentence listing ONLY the non-negotiable physical traits: hair as LENGTH + COLOR + TYPE-if-not-straight + BANGS-if-present (e.g. "long auburn wavy hair with soft wispy bangs", "short dark brown hair"), facial hair (if any), skin tone, approximate age, build (ONLY one of: thin/athletic/average), and eye color. Example: "TRAITS: Short dark brown hair, full beard, olive skin, mid-30s, athletic build, brown eyes." The hair clause must ALWAYS start with a length word (short, medium, or long). NEVER include how the hair is worn on the day (ponytail, bun, half-up, updo, braids, pin curls, "worn in", "pulled back", "swept back") or any part/hairline wording — the haircut is permanent, the styling is not. This line must be factual and compact — no adjectives beyond what\'s needed to identify the trait. IMPORTANT: never use negative phrasing like "no facial hair" or "no glasses" — AI generators interpret negatives as positives. If a trait is absent, simply omit it. For clean-shaven MALE faces, say "clean-shaven" instead of "no beard." For females, do NOT mention facial hair at all. Output ONLY the description, AGE line, and TRAITS line.',
