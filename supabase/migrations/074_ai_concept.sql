-- Migration 074: Add ai_concept JSONB to uploads
-- Stores the ConceptRecipe from Pass 1 of the two-pass engine.
-- Dream Like This extracts the "style" field directly instead of
-- guessing from the prompt text.

ALTER TABLE public.uploads ADD COLUMN IF NOT EXISTS ai_concept jsonb;

-- Add to get_feed RPC so feed data includes the concept
