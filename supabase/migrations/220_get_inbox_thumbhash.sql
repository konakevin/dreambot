-- 220_get_inbox_thumbhash.sql
--
-- Round 2 polish — extends get_inbox with a `upload_thumbhash` column so
-- the inbox notification thumbnails show a sharp blurry preview during
-- decode/load, same shape as the main feed (migration 219).
--
-- Body is otherwise identical to migration 202. Only the RETURNS TABLE
-- adds the new column and the final SELECT projects it (joined off
-- uploads.thumbhash already in scope via the existing LEFT JOIN).
--
-- get_friends_feed / get_following_feed are intentionally NOT updated
-- here — verified 2026-06-04 that they have zero client callers
-- (greppable across hooks/, app/, components/, lib/). Dead RPCs from
-- pre-2026 feed paths; no point migrating dead code.
