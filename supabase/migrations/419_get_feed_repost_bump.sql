-- 419_get_feed_repost_bump.sql — repost surfaces respect the one-time bump ledger.
--
-- Body is VERBATIM 391 (candidate windowing + impression penalty + jitter clamp,
-- all untouched) except the repost_agg CTE, which now:
--   • filters r.active = true       — un-reposted tombstones don't surface;
--   • filters r.activations = 1     — a re-repost (repost→un-repost→repost) stays
--                                     in the reposter's album but NEVER bumps the
--                                     feed again (the anti-manipulation rule);
--   • ages the surface off r.first_reposted_at (WRITE-ONCE, mig 418) instead of
--     max(created_at) — a re-repost can't reset freshness even in the edge case.
--
-- Net behavior: a repost floats a dream to the top of the reposter's FOLLOWERS'
-- feed once (following-tab recency reset + forYou social-proof boost, both keyed
-- on repost_agg). Only original posts compete in the wide forYou algorithm.
-- Locked by __tests__/db/repostBumpLedger.dbspec.ts.
--
...
AS $$
  ...
  -- Reposts of each upload by users the VIEWER follows. GROUP BY upload_id is the
  -- dedup + collapse (one row per upload, count + most-recent reposter). The
  -- one-time bump ledger (mig 418) gates this: only ACTIVE, first-activation
  -- reposts surface — a re-repost (activations>1) is album-only, and freshness
  -- ages off the WRITE-ONCE first_reposted_at.
  repost_agg AS (
    SELECT
      r.upload_id,
