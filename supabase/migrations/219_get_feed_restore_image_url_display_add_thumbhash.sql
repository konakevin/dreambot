-- 219_get_feed_restore_image_url_display_add_thumbhash.sql
--
-- TWO changes wrapped into one DROP+CREATE of get_feed:
--
--   1. RESTORE image_url_display in the return shape. Migration 211
--      (2026-05-30, model column) accidentally dropped image_url_display
--      from get_feed's RETURNS TABLE when adding `model`. Clients have
--      been getting undefined for that field for 5 days, falling through
--      to the full image_url (1-2 MB PNG instead of ~150 KB JPEG) on
--      every main-feed card. THIS IS THE ROOT CAUSE of the "black
--      full-screen cards" complaint — the main feed has been serving
--      full-res originals since 2026-05-30. Verified live:
--        SELECT key list of get_feed first row HAS NO image_url_display.
--
--   2. ADD thumbhash column to uploads + return shape. Kicks off the
--      thumbhash placeholder pipeline — expo-image's `placeholder` prop
--      shows a sharp blurry preview while the real image decodes/loads,
--      instead of a black or surface-tinted void. New renders persist
--      this (generate-dream / nightly-dreams / restyle-photo / bot
--      engine); legacy rows stay NULL until a later backfill pass.
--
-- Body is otherwise identical to migration 211 — only the schema row +
-- scored CTE projection + final SELECT changed.
