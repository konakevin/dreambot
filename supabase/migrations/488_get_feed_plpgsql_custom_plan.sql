-- 488_get_feed_plpgsql_custom_plan.sql — get_feed planned WITH its parameter values.
--
-- Diagnosis (2026-09-09): get_feed averaged 2.0-2.9 s server-side on every tab (6,500 calls since Sept 1)
-- while the SAME body run as a statement takes 0.4-0.6 s. A LANGUAGE sql function plans its body with
-- the parameters UNKNOWN (fmgr_sql plans with no bound values), so the optional filters
-- ((p_medium IS NULL OR …), (p_vibe IS NULL OR …), (p_bot_user_id IS NULL OR …), the p_tab CASEs) get
-- default selectivities: the planner expects a few hundred candidates instead of ~25,000 and picks
-- per-row index probes (108k buffer hits per call vs 7.7k). Proven: an all-parameter generic plan of the
-- body reproduces 107k buffers; a plpgsql copy of the SAME body, planned with the values, runs in
-- 385 / 307 / 140 ms (forYou / following / bots) with 10.4k buffers. Migration 460 (Sept 4) did not
-- cause this — the shape was identical in 448; the cost grew with the candidate set + impressions.
--
-- Change: body VERBATIM from mig 460 inside plpgsql RETURN QUERY; plan_cache_mode=force_custom_plan is
-- REQUIRED (with 'auto', a pooled PostgREST session would flip to the cheap-looking generic plan after
-- 5 calls and be slow again). #variable_conflict use_column keeps the RETURNS TABLE names from
-- shadowing column references. Output columns, order, cursor semantics: unchanged.
CREATE OR REPLACE FUNCTION public.get_feed(p_user_id uuid, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0, p_seed double precision DEFAULT 0.0, p_shuffle double precision DEFAULT 0.10, p_tab text DEFAULT 'forYou', p_cursor_score double precision DEFAULT NULL, p_cursor_id uuid DEFAULT NULL, p_medium text DEFAULT NULL, p_vibe text DEFAULT NULL, p_bot_user_id uuid DEFAULT NULL)
RETURNS TABLE(id uuid, user_id uuid, image_url text, image_url_hq text, image_url_display text, thumbhash text, width integer, height integer, caption text, description text, created_at timestamp with time zone, posted_at timestamp with time zone, username text, avatar_url text, allow_reposts boolean, allow_downloads boolean, comment_count integer, like_count integer, ai_prompt text, ai_concept jsonb, bot_message text, dream_medium text, dream_vibe text, model text, face_swap_mode text, repost_count integer, surface_type text, reposter_id uuid, reposter_name text, reposters_more integer, reposted_at timestamp with time zone, feed_score double precision, media jsonb, media_count smallint)
...
END
$$;
GRANT EXECUTE ON FUNCTION public.get_feed(uuid, integer, integer, double precision, double precision, text, double precision, uuid, text, text, uuid) TO authenticated, anon, service_role;
