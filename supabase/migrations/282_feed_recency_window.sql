-- 282_feed_recency_window.sql
-- ─────────────────────────────────────────────────────────────────────────
-- Audit A3/A4: get_feed scores the ENTIRE public-uploads set on every page
-- load (the score is computed at query time, so LIMIT can't be pushed into an
-- index → seq scan + sort on the hottest query in the app). Two changes:
--
--   1. A partial index on (posted_at DESC) WHERE is_public so the planner can
--      range-scan recent public posts.
--   2. Bound the forYou candidate set to a 365-day recency window BEFORE
--      scoring (forYou is recency-weighted — a >1yr post scores ~0 anyway, and
--      no post is anywhere near that old pre-launch, so ZERO visible change
--      now; it just caps the scored set as the table grows). 'following' and
--      'bots' tabs are unchanged. Followed-reposts of older originals are kept
--      (the `ra.upload_id IS NOT NULL` branch) so a recent repost of an old
--      post still surfaces.
--
-- Same signature/return as migration 261 → CREATE OR REPLACE (no DROP needed).
-- The ONLY change vs 261's body is the single recency clause in the `scored`
-- WHERE (marked below).
-- ─────────────────────────────────────────────────────────────────────────

CREATE INDEX IF NOT EXISTS idx_uploads_feed_recency
...
AS $$
  ...
  scored AS (
    ...
    WHERE up.is_public = true
      ...
      --    (keep followed-reposts of older originals). The only change vs 261.
      AND (p_tab <> 'forYou'
