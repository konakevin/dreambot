-- 389: clamp the forYou shuffle jitter to 0.15 (old-content flood fix).
--
-- Follow-up to migration 388 (impression discounting). The 0.45 manual-refresh
-- jitter (mig 352) was calibrated for a ranker WITHOUT a seen-penalty — brute
-- randomness standing in for principled variety. Once 388 suppressed
-- recently-seen posts, +-0.45 of noise dwarfed the remaining ~0.05-0.25 score
-- signal and the mid-feed became a uniform lottery over the entire 365-day
-- catalog: 71% of served posts were >30 days old (median age 51 days). Clamping
-- the jitter server-side restores recency dominance for ALL fielded clients
-- (older builds still send 0.45; LEAST() absorbs it): measured ~73-90% of the
-- feed under 48h with ~15-23% older discovery — the balanced mix. Body is
-- otherwise verbatim migration 388. Locked by feedSeenPenalty.dbspec.ts (a
-- shuffle 0.45 call must score identically to 0.15). Apply in the dashboard
-- SQL editor; replaces get_feed in place (same signature).
