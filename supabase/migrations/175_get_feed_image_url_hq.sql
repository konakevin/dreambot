-- 175_get_feed_image_url_hq.sql
--
-- Adds image_url_hq to the get_feed RPC return shape so the client
-- can decide whether to long-press save instantly (HQ cached) vs.
-- show the "this will take ~30s" confirm dialog (needs upscale) —
-- without a per-long-press DB roundtrip.
--
-- DROP + CREATE because Postgres won't let us add a column to a
-- RETURNS TABLE in place. Body is otherwise identical to migration
-- 127_bot_feed_support.sql — only the schema row + the two SELECT
-- projections changed (added image_url_hq).
--
-- Direct uploads SELECTs in hooks (useMyDreams, useSearchPosts,
-- usePublicProfilePosts, usePost, useAlbumPosts, useUserContextFeed)
-- are updated client-side in the same commit by adding image_url_hq
-- to their select() column lists — no DB work needed for those.
