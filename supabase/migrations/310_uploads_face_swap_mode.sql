-- 310_uploads_face_swap_mode.sql
-- ─────────────────────────────────────────────────────────────────────────
-- Track whether a dream was produced via a Dream-Cast face swap so we can
-- DISALLOW HD upscaling on it. Upscaling an already-rendered AI face forces
-- it into the uncanny valley, so cast dreams must never be offered "Save in HD"
-- (the native-resolution "Save to Photos" stays available — that's a direct
-- save of the existing pixels, no upscale).
--
-- Until now this fact only lived in ai_generation_log.rolled_axes.faceSwapResult,
-- which auto-prunes after 30 days (migration 274) → no durable signal the client
-- or the upscale-image edge fn can read. This column makes it permanent.
--
--   face_swap_mode:
--     'single'  → one cast face (self) was swapped in
--     'dual'    → two cast faces (self + plus_one) were swapped in
--     NULL      → plain render, no cast likeness in the final image (HD OK)
--
-- Gating is simply `face_swap_mode IS NOT NULL`. The single/dual distinction is
-- kept for forensics + future UX, not required by the current gate.
--
-- uploads uses COLUMN-LEVEL grants (migration 278) — a new column is invisible
-- to the client until granted, so we GRANT SELECT below. No client UPDATE grant:
-- only the service-role render fns ever write this.
-- ─────────────────────────────────────────────────────────────────────────

ALTER TABLE public.uploads
  ADD COLUMN IF NOT EXISTS face_swap_mode text;

GRANT SELECT (face_swap_mode) ON public.uploads TO anon, authenticated;

-- Backfill from the last ≤30 days of generation logs (older logs are pruned →
-- those uploads stay NULL = HD-eligible; cast dreams skew recent so the gap is
-- small and low-risk). The log does NOT carry upload_id (it's always NULL) — it
-- links to an upload via job_id → dream_queue/dream_jobs.upload_id. Only stamp
-- rows that actually carried a cast face into the final image; cascade/failed
-- swaps delivered an UNswapped scene → leave NULL. DISTINCT ON prefers 'dual'
-- when an upload somehow has more than one swap log row.
UPDATE public.uploads u
SET face_swap_mode = CASE
    WHEN agl.fsr = 'dual-success' THEN 'dual'
    WHEN agl.fsr IN ('single-fallback-success', 'success') THEN 'single'
  END
FROM (
  SELECT DISTINCT ON (upload_id) upload_id, fsr
  FROM (
    SELECT COALESCE(dq.upload_id, dj.upload_id) AS upload_id,
           l.rolled_axes->>'faceSwapResult'      AS fsr,
           l.created_at
    FROM public.ai_generation_log l
    LEFT JOIN public.dream_queue dq ON dq.id = l.job_id
    LEFT JOIN public.dream_jobs  dj ON dj.id = l.job_id
    WHERE l.job_id IS NOT NULL
      AND l.rolled_axes->>'faceSwapResult' IN ('dual-success', 'single-fallback-success', 'success')
  ) x
  WHERE upload_id IS NOT NULL
  ORDER BY upload_id, (fsr = 'dual-success') DESC, created_at DESC
) agl
WHERE u.id = agl.upload_id
  AND u.face_swap_mode IS NULL;

-- get_feed: add face_swap_mode so the client can suppress HD on other users'
-- cast dreams in the feed too. Adding a column to RETURNS TABLE changes the row
-- type, so CREATE OR REPLACE is rejected ("cannot change return type") — DROP
-- first. Same arg signature as migration 282; only the projection changes.
DROP FUNCTION IF EXISTS public.get_feed(
  uuid, integer, integer, double precision, text, double precision, uuid, text, text, uuid
);
...
AS $$
  ...
  scored AS (
    SELECT
      ...
      up.dream_medium, up.dream_vibe, up.model, up.face_swap_mode,
      up.repost_count,
