-- 211_uploads_model_column.sql
--
-- Adds `model` (TEXT, nullable) to the `uploads` table so the feed can
-- show which AI model rendered each post (Flux 1.1 Pro, Nano Banana,
-- GPT Image 2, etc.). Backfills existing rows from `bot_run_log.model`
-- where a join key exists, and updates `get_feed` to surface it.
--
-- Why nullable: legacy uploads + uploads from user-dream paths (Edge
-- Functions) that haven't been wired yet → silently hide the badge
-- client-side when null. We can fill these in a follow-up.
--
-- get_feed: DROP + CREATE because Postgres won't let us add a column to
-- a RETURNS TABLE in place. Body is otherwise identical to migration
-- 175 — only the schema row + the two SELECT projections changed
-- (added `model`).
--
-- 2026-05-30.
...
-- Backfill from bot_run_log via image_url (bot_run_log doesn't have an
-- upload_id FK; the same image_url it logged for a successful run is the
-- one stored on the corresponding uploads row). Only join successful runs
-- — failed/safety-rejected rows have no real image_url. Older posts whose
-- bot_run_log row predates the model column stay null.
UPDATE public.uploads u
SET model = brl.model
FROM public.bot_run_log brl
WHERE brl.image_url = u.image_url
  AND brl.status = 'ok'
  AND brl.model IS NOT NULL
  AND u.model IS NULL;

-- ── get_feed: re-create with the new `model` column in the return shape ──
