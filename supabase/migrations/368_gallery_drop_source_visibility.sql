-- 368_gallery_drop_source_visibility.sql (2026-07-11)
--
-- Reverts the per-source visibility model from migration 367. Albums compose
-- from the user's PRIVATE (unposted) dream pool, so "member visible only if its
-- source is public" hid every image of every album (Kevin 2026-07-11). New
-- model: an album is a published SET — its members always show while the album
-- post itself is public; deleting a source still drops it (cascade + heal
-- triggers, KEPT). This drops: the source-hide cover trigger, the RLS source
-- predicate, and the get_feed/get_shared_post visibility predicates.

BEGIN;

-- 1. Source-hide cover trigger is now meaningless (a private source stays in
--    its album) — drop it so it can't churn covers.
DROP TRIGGER IF EXISTS trg_refront_galleries_on_source_hide ON public.uploads;
DROP FUNCTION IF EXISTS public.refront_galleries_on_source_hide();

-- 2. RLS back to host-only visibility (migration 356 verbatim): a member is
--    visible if the album post is public OR owned by the caller.
...
-- ── get_feed / get_shared_post: revert per-source visibility (migration 368) ─
-- Album members always show while the album post is public (reverts the
-- 367 source-visibility predicates). Media subquery back to host-only; the
-- zero-visible-member host filter removed.
