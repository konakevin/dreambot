-- 388: restore per-viewer impression discounting in get_feed (stuck-posts fix).
--
-- THE AUDIT FINDING (2026-07-21): feed v3 (migration 104) had a per-user
-- seen-post penalty driven by post_impressions; the 107->127 rewrite era
-- dropped it and eight subsequent get_feed revisions carried the loss. The
-- impressions pipeline never stopped recording (33k+ rows; the reporting user
-- had posts served 34x) — the ranker just never looked. Symptom: the same
-- followed/popular posts hold a permanent ~0.30 score baseline that recency
-- jitter (±0.10 cold) can never unseat, so they re-serve every session for
-- weeks.
--
-- THE FIX: multiplicative seen-penalty with time recovery (forYou/explore:
-- x0.55 / x0.35 / x0.20 at 1 / 2 / 3+ views; following: x0.75 / x0.50; bots
-- timeline: none), relaxing linearly to full strength over 21 days since
-- last_seen so deliberate "callbacks" to old posts still happen.
--
-- PURELY BACKEND: same RPC name/args/return shape — every client in the field
-- benefits the moment this is applied. Mid-session cursor drift from freshly
-- recorded impressions is absorbed by the client's cross-page id dedup.
-- Body is otherwise verbatim migration 386. Locked by
-- __tests__/db/feedSeenPenalty.dbspec.ts so no future rewrite silently drops
-- it again (how it was lost last time). Apply in the dashboard SQL editor.

-- Composite index for the per-viewer join: the UNIQUE(user_id, upload_id)
-- constraint from migration 090 already backs it; this is belt-and-suspenders
-- in case that constraint is ever relaxed.
CREATE INDEX IF NOT EXISTS idx_post_impressions_user_upload
  ON public.post_impressions (user_id, upload_id);
            ...
            + (1.0 - CASE p_tab
                ...
                ELSE
                  CASE
                    ...
                    ELSE 0.20
                  END
              END) * LEAST(scored.days_since_seen / 21.0, 1.0)
          )
