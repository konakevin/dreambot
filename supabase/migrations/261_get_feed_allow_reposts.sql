-- 261_get_feed_allow_reposts.sql — expose the author's allow_reposts on feed rows.
--
-- The repost button must hide on posts whose AUTHOR has opted out of reposts
-- (users.allow_reposts = false). The enforce_repost_rules trigger (migration 242)
-- already blocks the INSERT, but the CLIENT had no way to know, so the button
-- still showed and tapping it failed + rolled back. This threads the author's
-- allow_reposts onto every get_feed row so the client can gate the button.
--
-- Adding a RETURNS TABLE column is a return-type change (42P13) → DROP first.
-- Verbatim re-create of the migration 253 body with `allow_reposts` added in
-- three spots (RETURNS TABLE, the `scored` select, the final select), right
-- after avatar_url. Run in the dashboard SQL editor.
