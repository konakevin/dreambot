-- 390: age-aware seen-penalty waiver (the "all I see is bots" overcorrection fix).
--
-- Follow-up to 388/389. The full-strength penalty on seen posts overcorrected
-- for DreamBot's catalog size: an active user exhausts the ~72-posts/day fresh
-- pool daily, so once friends' fresh posts were seen ONCE they sank to the old-
-- catalog band and the un-penalized remainder was the bot firehose. Kevin's
-- spec (2026-07-21): repeats are fine for a post's first few days; only week+
-- old posts must never stick.
--
-- Change: the seen-penalty's recovery blend becomes GREATEST(time-recovery,
-- youth) where youth = 1 - post_age/72h. A brand-new post carries ~no penalty
-- however often it's been seen; the waiver tapers linearly to zero at 72h,
-- after which behavior is IDENTICAL to 389 (full penalty, 21-day recovery).
-- The original stuck posts were WEEKS old — their suppression is untouched.
-- Self-correcting as supply grows: more human posts per day = more unseen
-- content inside the 72h window = repeats fade naturally.
--
-- Body otherwise verbatim 389. Locked by feedSeenPenalty.dbspec.ts (young-post
-- waiver ratio + the unchanged old-post penalties). Apply in the dashboard SQL
-- editor; replaces get_feed in place.
