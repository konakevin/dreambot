-- 460_get_feed_restore_media.sql — restore the `media` (+ `media_count`)
-- gallery columns that get_feed silently lost.
--
-- Migration 356 added a trailing `media` jsonb column (+ correlated
-- json_agg subquery over upload_media) to get_feed so multi-photo/album
-- posts carry their extra slides into the home feed. Migration 386
-- ("add face_swap_mode") redefined get_feed by copying migration 378's
-- BODY VERBATIM to layer face_swap_mode on top — but 378 predates 356, so
-- that copy silently dropped the media column. Every get_feed edit since
-- (388, 389, 390, 391, 419, 448 — the current version) carried the
-- gallery-blind shape forward; none of them ever restored it.
--
-- Net effect: DreamCard's isGallery gate (`item.media.length > 1`) is
-- ALWAYS false on the home feed, even for genuine album posts, because
-- `row.media` is undefined → mapMedia() returns []. Profile views are
-- unaffected (they fetch via POST_SELECT's upload_media embed, not
-- get_feed) — that's why the same post shows swipe/dot controls on a
-- profile but not on the home feed.
--
-- This is a pure restoration: same FROM/filters/scoring as mig 448
-- (verbatim), same media subquery shape as mig 356 / get_shared_post
-- (mig 449) — just re-attached to the current final SELECT (which now
-- joins `ranked` to `uploads up` directly rather than selecting off a
-- `final_scored` CTE, so the subquery correlates on `up.id`). Also adds
-- `media_count` (a plain uploads column) since lib/mapPost.ts's
-- mapRpcToDreamPost has been reading both fields off get_feed rows all
-- along (harmless when absent — defaults to 1 — but now correct).
--
-- Run in the Supabase dashboard SQL editor (DDL) via scripts/apply-migration.mjs.
