...
-- Bot feed support
--
-- 1. is_bot column on users (identifies bot accounts)
-- 2. get_bot_users() RPC (client fetches bot list for pill row)
-- 3. Extend get_feed with p_bot_user_id param + 'bots' tab
-- ══════════════════════════════════════════════════════════════════════════════
...
UPDATE public.users SET is_bot = true WHERE username LIKE '%Bot';
...
-- ── 3. Extend get_feed with bots tab ────────────────────────────────────────
...
AS $$
  ...
  scored AS (
    ...
    WHERE up.is_public = true
      ...
      -- Skip self-exclusion for bots tab (Kevin owns bots)
