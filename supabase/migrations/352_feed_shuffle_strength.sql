-- 352: seed jitter becomes a caller-tunable SHUFFLE STRENGTH.
--
-- "Reseeding" the feed barely moved it: the seed contributes only a 0-0.10
-- jitter on computed_score, so any post whose engagement/recency score leads
-- by more than 0.1 stays #1 under EVERY seed (Kevin 2026-07-09: "the first
-- pic never changed"). New p_shuffle param = the jitter weight. Default 0.10
-- preserves today's cold-load ranking exactly; manual refreshes (Home re-tap
-- / pull-to-refresh) pass 0.45 so a reshuffle genuinely reshuffles.
--
-- The old 10-arg signature is dropped first — leaving both would create an
-- ambiguous overload for PostgREST rpc calls.

DROP FUNCTION IF EXISTS public.get_feed(
  uuid, integer, integer, double precision, text,
...
-- (signature includes the new p_shuffle param — the copied 10-arg grant was
-- the 42883 on first apply)
