...
| 23 | MangaBot | onsen-evening | 5+1 | **~4.6 clean / 2-of-6 board text → board DELETED, re-validating** | Graded the previously-unread validation batch: 4 of 6 clean and lovely (a painted crane on the door, a painted maple leaf + pine sprig on the entry board, snow monkeys with a baby in a leaf hat, blank lantern faces) but **2 of 6 put gibberish lettering on the SIGN BOARD** (one small, one a prominent kanji-style board over the entrance). Exactly playbook 12 + 23: a board is a text-SHAPED surface, so "a board bearing one painted X" is unsafe however it is worded, while the noren rendered clean 6/6. Fix: the sign board deleted from all three places it lived (20 pool clauses, template rule 5, output order), its motifs moved onto noren / doorway plank / lantern faces — the surfaces the playbook already measured clean 9/9. Eave boards and planks kept |
