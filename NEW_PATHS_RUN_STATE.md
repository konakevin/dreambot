...
| 23 | MangaBot | onsen-evening | 6+1 | **~4.5 clean, 2/6 board text — lintel fix validating** | Graded two previously-unread batches. 4 of 6 clean and lovely both times (a crane on the noren, a maple fan, snow monkeys with a baby in a leaf hat, sparrows on a bamboo rail, a go board by the water) but **2 of 6 put gibberish on a SIGN BOARD** each time. Deleting the board noun from all three layers did NOT move the rate (2/6 → 2/6): it came back uninvited, because a Japanese inn entrance IS a signboard prior, and my replacement wording ("only deep eave shadow") was an ABSENCE, which lesson 26 says is exactly what gets backfilled. Composed fix now rendering: delete the noun AND fill the lintel positively (a row of round lanterns / a hanging pine bough / snow and icicles, never flat, never rectangular). The noren carried its painted motif correctly 11 of 12 across both batches — the dividing line is whether the object's SHAPE is a sign, not whether it is flat |
