...
| 5 | BloomBot | alpine-wildflower-meadow | 4 | **PASS 4.67 (min 4.5) — BEST OF THE RUN, AWAITING GRADE** | R1 3.54 → R2 4.13 → R3 4.13 → **R4 4.67**. A 5.0 render: a crevassed BLUE GLACIER TONGUE hanging above the flower line with magenta fireweed in the rock seams below it. Zero text, zero humans, zero garden species, zero corridors in 24 renders; ultra signed 0 of 13 so no pin needed. **It also cracked the bot's structural cap:** `BLOOM_NEUTRAL` sits at words 40-78 and mandates "blooms FILLING THE FRAME as the unmistakable hero" — a frame-packing instruction in the attended region, and R1's two worst renders were that instruction executed (a carpet with a peak behind it = the existing `flower-fields`). The bot's own heroMandate hatch is in the Sonnet brief and cannot reach a Flux-side fragment. Fix = code-only per-path medium: carpet 2/6 → 0/6, and buying back 13 preamble words took look-landed 0/6 → 4/6. Residual: the ground can tip too far to rock (~1 in 12); lever = state the split as a committed fraction in the fragment |
| 6 | BloomBot | coastal-cliff-bloom | 4 | **4.13, NOT a pass, AWAITING GRADE** | R1 3.67 → R2 4.00 → **R3 4.13** → ultra-pinned probe 3.92. One 5.0 render: standing on the rock at the edge, orange lichen over grey stone, magenta+purple+yellow flowers, turquoise water below with foam, edge flowers silhouetted against it, and a big **bone-brown dead plant beside living ones** — the salt fact rendered exactly. Plus a 4.75 woodblock with every stem leaning hard out over the water. Residual: **the postcard vista, ~4 of 6, and it is NOT beatable by wording** — `cliff` reached 0 of 24 emitted prompts and the whole cliff still rendered. Secondary: colour salad (Sonnet mixes 6+ hues). Lever = cut the emitted prompt ~350 → ~180 words by deleting template prose and output-order items (5 rule blocks, 9 order items, a 6.6 KB template); measured on this path, **everything at 5-10% of the prompt rendered 6/6 and everything past ~30% rendered 0/6 until moved**, so halving it is the only way the sea/light/charm axes reach the attended third without stealing the prefix from the wind facts |
| 7 | BloomBot | orchid-cloud-forest | 3+1 | **4.42 (floor 4.0), CLOSE, AWAITING GRADE** | R1 3.92 → R2 4.13 → **R3 4.42** + a 3-render xerox confirmation. Two 4.75s, one of them "genuinely never seen before, unlike anything in BloomBot's 24 paths" (white cascades and egg-yellow on a trunk). Measured movement: hard cumulus-sky/valley vista **3/6 → 0/6**, mist as an eye-level volume **2/6 → 6/6**, species-colour fidelity **1.5/6 → 6/6**, pink-dominant frames 5/6 → 2/6; on-wood premise 21/21, ultra signed 0/21. Residual: emitted median **377 words** vs a 250 fleet median, and both remaining soft defects look like first-third starvation (orchids scatter like stickers instead of clumping; a pale sky patch survives) because "clustered in one moss pad" and "nothing overhead but wet green" both sit LATE in the order. Lever = delete template prose and order items, per lesson 43 |
...
| 10 | BrickBot | archaeology-dig | 3 | **3.45, not a pass, AWAITING GRADE** | MERGED shadow, 18 renders. R1 2.32 → R2 3.10 → **R3 3.45**, with the model mix held EXACTLY constant (3 pro + 3 ultra every round) so each round is a paired comparison — good discipline. **Text 0 of 18** with zero text-prior nouns in any emitted prompt, on the fleet's densest label-magnet subject, won by a rounded-kit-only design (baskets, barrels, sieves, sacks, coils of rope; crates/boards/trays/labels deleted from every layer). Reference frame 4.4: a colossal white brick hand rising out of the trench floor with four crew standing ON the open palm, a knotted rope with roots down the wrist, banded walls both sides, teal sky. **R1 was a NULL and the agent correctly refused to encode it** (pro 2.43 vs ultra 2.20 at n=3/arm, below resolution, lesson 42) — so the model entry ships as a 50/50 split, not a pin. R2's variable was the WRAPPER (→ lesson 73): 83 words of fixed preamble in three stacked layers put the scene start at 19-34% even though every law was already in 6/6 prompts; a 28-word path-own medium took it to 8-16%, colour bands 1/6 → 6/6, corridors 4/6 → 1/6. R3 folded the scale ruler into output-order item 1 and DELETED the standalone crew item, taking figures 2/6 → 6/6 while median words FELL 409 → 355 (→ lesson 74). Residual: **the find renders 0 times in 3 of 6**, and 2 of those 3 trace to ONE entry of 25 — a giant ladder, while every `cut` entry also puts an ordinary ladder in the trench, so the mundane instance substitutes for the hero (→ lesson 75). Lever = purge that entry and filter any `find` sharing a noun with the cut's way-down or the camp kit. Deliberately unspent: it would land unvalidated on the round being graded |
...
| - | DinoBot | tidal-flat-tracks | 3 | **R2 4.02 is the best round (R3 3.54), AWAITING GRADE** | MERGED shadow, 15 renders, 0 errors. ⚠️ **Grade R2, not R3** — R3's variable genuinely fixed the grey frames (colour-vs-neutral 6/25 → 0/25, grey frames 2/5 → 0/5) but a SEPARATE 3-entry defect rolled twice and cost the round: sided-sky entries assign hue-left/hue-right, which is a split-panel instruction (present → panel 2/2, absent → 0/3, lesson 63). **Reference frame r2#3 ~4.9:** a giant three-toed hollow cropped by the bottom edge with dozens of amber-shelled periwinkles streaming down its inner slope and ringing the pooled water, the trackway marching away along the tideline, a black squall wall with a burning saffron gap laid flat again in the standing water — a whole colony has moved into a dinosaur's footprint while a storm comes in. **R1 was the round that decided the path** (lesson 33, 4th confirmation on this bot): every structural law reached 5/5 emitted prompts and three of five models STILL deleted the premise, each reproducing its own documented amber-forest behaviour — hence `modelByPath` 60/40 flux-2-pro/gemini. Also needed a NEW `sensoryAnchors.skipPaths` (verified honoured at sensoryAnchors.js:152, and verified to still fire for other paths): `requiredChannels: ['lightcolor']` is a second palette source on 100% of renders, a third of its default `scene` pool names a PLACE, and all 15 graded renders ran with it off. Text 0/15, `beach` 0/15. **Two measured levers, both pool-only, deliberately unspent at the 3-round cap:** (1) purge the 6 of 25 `evidence` entries that are a CONTINUOUS LINEAR feature — discrete hollows mean 4.19 vs continuous 2.83, and R3 separates perfectly (continuous → corridor/panel 3/3, absent → 0/2), because a furrow IS a corridor however the crosswise law is worded; (2) reword the 3 sided-sky entries to layers |
...
| 15 | FaeBot | autumn-seed-gathering | 3 | **4.15, CLOSE not a pass, AWAITING GRADE** | MERGED shadow. R1 2.27 → R2 3.33 → **R3 4.15** (min 3.4), 18/18 delivered on flux-1.1-pro, zero E005, zero signatures, **zero readable text, zero putto** — it beat the trap that sank `acorn-boat-regatta` by inheriting honey-harvest's opener. Emitted prompts held flat at a 333/329/336-word median, so neither gain was bought with length. **The headline is a near-miss on a wrong conclusion** (→ lesson 70): R1's laws were all present AND correctly placed at 1-11%, which by lesson 33 means a MODEL fact, and the next step was a flux-2 probe that honey-harvest had already proven unshippable. The assembled prompt showed why — 35 words of the BOT-WIDE register (*"soft ethereal painterly … dreamy atmospheric painted glow"*) at 18-25%, the literal opposite of a wind rodeo, on 6/6. A path-own register of the SAME length took costume 0/6 → 4/6, saturated palette 0/6 → 6/6, near-nudity 2/6 → 0/6. R3's variable was ONE token: `winged` → `papery blade`, because `wing` referred to a SEED 34× and to the FAE 33× across 175 entries and flux cannot bind it to the seed (→ lesson 71); winged-seed prompts 5/6 → 0/6, blade-seeds 0 → 5/6. Reference frame 4.6: a fae in a green leaf-cloth work dress hanging off a colossal split pod at full stretch, one arm flung wide, hair streaming, the air solid white with tufts, saturated gold field against blue. ⚠️ **Grade R3 only** — R1/R2's 12 renders are still on the profile per the no-delete policy and are the pale sober batches. Residual: everything the POOLS contribute still lands in the dead zone (pod mechanism at 56% → ~2/6, backlit halo at 74% → ~1/6) while everything in the PREFIX lands at 1-11% → 4-6/6. Lever = DELETE output-order item 4 (the stated air count), because the prefix already carries "two hundred white seed tufts" on 6/6, which promotes the pod and the light into the attended half at zero cost |
...
| - | ↳ | E005 root cause — SETTLED, and my retraction was ALSO wrong | — | **Path-specific after all. Two over-corrections, final answer below.** | Sequence, so the reasoning is auditable: (1) I first concluded apiary's failures were path-specific content, on apiary 2/14 vs farmbot's other paths 0/586. (2) The agent then reported every failure carrying the IDENTICAL Replicate id `uIJ6l3ruRD`, and its clothing-reword arm went 0/4, so I RETRACTED and called content unsupported. (3) Then I pulled the full history — **1,192 flux-2 runs** — and the retraction was wrong too: `star-charting` **10/13 (77%)**, `apiary-beekeeping` **3/26 (12%)**, and **every one of twelve other flux-2 paths is 0 E005 across 400+ runs**, flux-2 overall 13/1192 = 1.1%. A concurrent unrelated flux-2 control ran clean. **My error in step 2: `uIJ6l3ruRD` appearing in 13 of 13 is far better explained as a STATIC id inside Replicate's canned E005 message than as a cached per-request response — I treated a constant as a signal.** So: the trigger IS specific to these two paths. What they share and the twelve clean paths do not is a CLOSE HUMAN/HUMANOID BODY (an adult fae described in physical proportion; close-range human hands among bee swarms); the clean ones are landscapes, towns, food and vistas. But the reword arm going 0/4 means it is NOT the clothing clause specifically — so it is the pose, the proportion vocabulary, or the combination. **Next experiment, one variable each: ablate the POSE axis, then the proportion clause, 8 attempts per arm.** Still do NOT blanket-soften the astronomer pool: its adult-proportions law is what beat the putto trap that cost acorn-boat-regatta 5 of 6 renders |
| 18 | FaeBot | honey-harvest | 3 | **~4.2 (two 4.5s), AWAITING GRADE** | MERGED shadow. Beat the naked-putto trap that sank `acorn-boat-regatta` **6/6 on the first try**: the fix is a `promptPrefixByPath` leading with *"one slender grown fae hard at work, caught mid-movement"* — a STATIC opener rendered the work beat 0/12 while sitting in 12/12 prompts, and adding the verb additively took it to 6/6. **Pinned `flux-1.1-pro`**, which CLOSED star-charting's open blocker as a flux-2 FAMILY fact: flux-2-flex hits the identical E005 wall as flux-2-pro, while flux-1.1-pro is 23/23 delivered on FaeBot with zero E005 |
...
**⚠️ apiary-beekeeping — INCOMPLETE FIX CLOSED 2026-09-23 (see lesson 60), UNVERIFIED.** The giant-bee inflator had been removed from the template prose and the pools but NOT from the `WRITE THE PROMPT IN THIS ORDER:` line, which is the one layer emitted on 100% of renders. That line still read *"detail on the nearest one or two only"* while the prose twelve lines above it said *"none is singled out for close anatomical detail, because detail and size are the same dial"* — a direct self-contradiction, with the winning layer holding the harmful half. Now reads *"every bee including the nearest one a small clean shape at the size named"*. **The 4.37 grade was measured BEFORE this fix**, so the grade stands as the honest record of what was rendered; the fix is expected to help and needs one 6-render round to confirm. Found by the `sheep-shearing-day` agent auditing a sibling path, not by me.
...
| 21 | FarmBot | sheep-shearing-day | 3 | **~4.4, AWAITING GRADE** | MERGED shadow. **18/18 renders succeeded with ZERO Replicate content-filter failures**, notable because the two sibling close-range FarmBot paths trip it at 12-17% and this one has blades and hands among animals. The entire QA arc was ONE defect (the bare shorn sheep) — text 0/18, blood/red 0/18, no giant animals, no diorama voids, no split panels, correct scale throughout, all holding from round 1. **The bare body is the run's best measurement** (→ lesson 58): additive "woolly cuffs still at the ankles" rendered 6/6 every round while subtractive "no fluff on the body anywhere" rendered 1/6, SAME sentence and position; reframing it as a positive object (a GREYHOUND-shaped body WEARING a woolly hat and four socks, odd-one-out against a woolly majority) took it 0 → **5/6** with the six pools byte-identical between rounds. Best frame 4.7: one bare greyhound-shaped animal in a woolly hat dead centre of the mob, steam, lamp, bundle, broom. Second-shortest path on the bot (median 182 Sonnet words vs FarmBot's 562), though the scene still starts at 58% of the emitted prompt because of the 273-word shared fragment. Residual: bare body 5/6 not 6/6; lever = ALSO put that clause in the framing block, the one place a FarmBot path can reach early |
| 22 | FarmBot | hay-baling-summer | 3 | **R3 3.97, not a pass, AWAITING GRADE** | MERGED shadow (2 lines, no pins, no skips — verified: farmbot declares no chaos/polish/sensoryAnchors/mediumByPath AND has no `bot_config` row, which matters because the overlay would otherwise switch polish on with an undefined skipPaths). 18/18 rendered, 0 content-filter failures. R1 3.83 → R2 3.80 → **R3 3.97**. Text **0/18**, corridor **1/18**, giant-object failures 0/18. Reference frame 4.8: an enormous cropped bale, hot rim against a deep indigo flank, its shadow a hard bar across the frame, the whole air packed with individually lit motes, a twine-bound ridge with one pheasant feather laid on it. **R2's variable is the run's best bookend** (→ lesson 77): trimming for length had deleted the hard-LINE half of the crosswise law FROM THE OUTPUT ORDER while leaving it in the prose and 24/25 field entries — it reached **0 of 6** prompts, and the path's differentiator was weak or absent in 4/6 renders, while the OTHER half of the same law, which survived in the order, reached 6/6 and held corridors to 0/6. Restoring it to the order alone: 0/6 → 5/6 → 6/6. **This is the exact inverse of the apiary bug fixed the same day**, so the loop is now closed in both directions. ⚠️ **R3's own variable did NOT ship** — the singular-cast clause reached 0 of the 3 character-branch prompts, so R3 ≡ R2 in effect plus variance; kept because it matches both proven siblings, but unverified. Residual: **flat uncommitted light in 7/18**, and the agent correctly ruled out the obvious 'move it earlier' round by measuring clause position against palette score across 12 renders (no relationship: 78% → 5.0 but 64% → 2.5). Isolated instead to ONE family of ONE pool — FLAT-HIGH-HEAT at 2.50 mean vs 3.00-4.33 for every other family — whose own wording is *"lies flat"*, *"laid flat across"*, *"bleached"*. Self-inflicted and measurable: rewrite ~5 entries to name a hard-edged shadow on a named surface, keep HAZY-AIR (4.00) since softness is not the problem |
...
| 24 | MangaBot | game-center-arcade | 3+1 | **PASS ~4.0, AWAITING GRADE** | R1 3.32 → R2 3.35 → **R3 4.03** → confirmation 3.97, final 12 mean 4.00. Best 4.6: a young woman feeding her last coin in with three more stacked on the ledge and one already rolling, amber against a cold teal shaft and near-black, zero text. Corridor **3/6 → 0/12**; every machine in shot FULL 1/6 → **12/12**; named actor 10/10. MangaBot's first true interior-night path (onsen was kept open-air to dodge exactly these traps). Medium fragment measured at 56 words, scene starts word 58 — a pure style register with no mandate, so **no path-own medium needed**. Residual: lettering 7/12 but prominent in only 2, now confined to the compressed background band; lever = fill that band with a named physical mass (ducts, a prize-shelf wall of plush, a mirrored soffit). It also declined to encode a look-filter on n=5 at a 0.4 effect, per lesson 42, and recorded the 8-vs-8 experiment instead |
| 25 | PixelBot | castle-town-gate | 3 | **PASS 4.64 (min 4.3), AWAITING GRADE** | R0 3.02 → R1 4.46 → **R2 4.64**. Best 4.9: a timber-framed gatehouse three-quarters on, red-lit half-timbered houses stepping up behind the wall with a fat white chimney plume, the winding wheel and counterweight in plain view, a figure on the walkway pointing up at the plume while another shades their eyes, a plumed bird on a rein. Across 14 delivered prompts: text/device nouns **0/14**, off-centre three-quarters 14/14, travellers with a welded ruler 14/14, crosswise wall law 13/14, town above the wall 12/14. The delete-the-noun + fill-the-lintel composition is the build's strongest result — the lintel came back FILLED in 10 of 11 renders (a bell, a flower bundle on red cord, a pine bough under snow, a birdbath, a gourd hoist, a mossy lion) and never flat or rectangular. Residual: a carved head named "BESIDE the arch" gets mirrored by flux's gate-symmetry prior (beside 1/2 and gatepost 1/1 mirrored, **above 0/3**); lever = move every head to ABOVE and delete "beside" from the pools |
...
| 27 | PixelBot | observatory-tower | 3 | **4.26 CLOSE, AWAITING GRADE** | MERGED shadow. R0 4.42 → R1 3.96 → R2 4.26 (min 3.0, max 5.0). Reference render 5.0: a tall figure on the ladder handing a STEAMING MUG down to a small figure with both arms up to receive it, hanging coloured weights on cords at graded heights, saturated gold stone against a magenta star-slot. **Text-prior nouns 0/15** on the fleet's highest text-risk subject (a wall of little drawers rendered with no labels), astronomy jargon 0/5 in R2 — won by moving every pinned object off a flat cork board onto the tower's own CURVING plaster (lesson 57). R1 is a textbook lesson-34 self-violation: +40 brief words bought the narrow slot 3/5 → 5/5 and pushed `reading_tool` + `room_dressing` off the end, giving 3 of 5 bare-walled rooms, 4.42 → 3.96; R2 paid for the same clause by DELETING template prose instead and recovered. Residual = **length, quantified** (lesson 54): the 8 shortest prompts avg 4.51, the 7 longest 3.87, everything ≥323w graded 3.0-3.6. Lever = trim the 186 of 225 over-cap pool entries (not the medians); unspent because it is pool work at the 3-round cap |
...
| 29 | SteamBot | rooftop-telegraph | 3 | **PASS 4.20 (best 4.8), AWAITING GRADE** | R1 3.22 → R2 3.72 → **R3 4.20**. Reference render 4.8: two cream signal discs face-on on a brass spindle above a brick tower, a signaller working the lever on the gallery, a second figure walking the snowy gutter toward him, amber gaslamps on snow against a cold pale-blue city of smoking chimneys. Hardware in frame 0/6 → **6/6**; clock/dial faces 3/6 → **1/6**; corridor 3/6 → **0/6**. R1's diagnosis was NUMERIC not visual: the roofline noun landed at word ~104 (25%) and rendered 6/6, the blade word at ~177 (45-51%) and rendered 0/6. R3's variable was a path-own medium, after proving the clock prior's source — `clockwork`+`gears`+`gauges` in the SHARED wrapper in 12 of 12 prompts with zero such tokens in the path's own text. Residual: the vantage reaches only 1 of 6 prompts (and that render is the 4.8) because it sits at a seed TAIL; lever = give the vantage its own output-order item |
...
| 32 | ToyBot | sand-toy-beachworks | 3 | **3.5, CLOSE not a pass, AWAITING GRADE** | MERGED shadow, 18 renders, 0 errors, all flux-1.1-pro. R0 2.5 → R1 2.8 → **R2 3.47**. **The premise now lands 6/6 where it landed 1/6**, and the measurement is the cleanest positional intervention of the run (→ lesson 65): the first tool noun sat at 39-81% of the emitted prompt and rendered 1 of 6; moved to 5% it rendered **6 of 6**, one variable, additive, prefix only, nothing removed. It bought saturated colour **0/6 → 4/6** for free, because the one bright object in a neutral sand frame IS the colour. Also: lettering **0/18** on a path made of branded plastic, humans 0/18 after R1, corridor 5/6 → 3/6. Best frame 3.9: a cathedral-scale sand fortress crossing the frame with a wet moat at its base and a big orange bucket low right, so the picture says *all of this came out of that*. **Ultra pinned out from round 0 on the SIBLING's evidence rather than a fresh probe** (bath-toy-flotilla got 4 framing failures from 4 ultra renders on the same class of camera-height identity) — good lesson-42 discipline, no round wasted. Residual: the toys render factory-fresh glossy 6/6 even though the matte finish IS in every seed's opening noun phrase, because that text lands at **86-91%** of the prompt — the same dead zone the tool was in (→ lesson 66, the two rules compose). Lever: extend R2's exact move to the finish, six additive words on the prefix's own bucket only, leaving the seeds theirs |
...
**In flight:** none — **all 35 built, merged and awaiting Kevin's grade.**
**Wave 8 (last 3):** BrickBot `archaeology-dig` · FaeBot `autumn-seed-gathering` · FarmBot `hay-baling-summer`
...
| `archaeology-dig` (BrickBot) | `western` (arid), `macro-display` (close-up brick), `lego-landscapes` | EXCAVATION AS A VERTICAL CUT: the trench wall showing colour-banded strata, a string grid over the pit, spoil heaps, tents and tools. The hero is the PARTIAL reveal — something large still mostly buried, so the viewer completes it. Not a museum display, not a desert vista |
| `autumn-seed-gathering` (FaeBot) | ⚠️ **highest collision left in the run** — `acorn-boat-regatta` (already owns ACORNS, and is the bot's worst path at 3.07), `honey-harvest` (a gathering-work path, just built), `goblin-market` (trade in goods), `flower-fairy`/`tiny-fae` | SEED DISPERSAL PHYSICS as the show: dandelion parachutes, maple samaras spinning, burst milkweed floss, burrs that CLING, seed heads that spring and rattle. The fae are catching things that fly or stick, so the path is about motion the other paths cannot show. **Acorns are banned — that hero belongs to `acorn-boat-regatta`.** AUTUMN also differentiates: the bot's roster is green/spring/summer plus one winter court. **MUST carry honey-harvest's proven opener** (a fae AT WORK mid-movement, not a static description) — that is the measured fix for the naked-putto prior that sank `acorn-boat-regatta` 5 of 6 |
| `hay-baling-summer` (FarmBot) | `harvest-festival`, `picnic-in-the-meadow`, `flower-field-wandering`, `evening-chores` | SCALE, GEOMETRY AND LABOUR: round bales as huge cylinders throwing long shadows down stubble rows, the hard cut/uncut line across the field, a wagon stacked high, backlit chaff in the air. Hot, dusty, industrial-scale work — not a stroll and not a festival. The hero is THE FIELD TRANSFORMED into a receding pattern. Note FarmBot's shared fragment spends the first 273 words, so the differentiator has to be placed early (lesson 34) |
