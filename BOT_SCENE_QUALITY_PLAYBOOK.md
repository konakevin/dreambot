...
**Nano Banana carries the looks (the ChibiBot LESSON 2 path, re-confirmed).** Banana was initially routed to `bloombot_gpt_clean` via `cleanMediumByModel` (no look) — ~45% of a batch came back look-less. Fix: REMOVED `google/gemini-2-image` from `cleanMediumByModel` entirely so it renders `bloom_hyperreal_cgi` (neutral) + the rolled look directly. Verified on a 6-render Banana-forced pass (`iter-bot --model google/gemini-2-image`): **zero abstract-plate drift**, and several Banana renders (ink-linework storybook / botanical-illustration registers) were the prettiest of the session. Confirms ChibiBot's finding that gpt-2/banana handle CONCRETE style tags (oil / watercolor / woodblock / stained-glass) with no drift — the old abstract-plate risk was the painterly-FUSION mediums, not concrete looks. (For BloomBot, removing Banana from the map outright beats `skipPaths` because looks apply to ALL paths — there's nothing to skip.)
