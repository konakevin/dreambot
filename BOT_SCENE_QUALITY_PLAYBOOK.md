...
**Status:** R5 converged (Kevin liked the look but held it back). Path DEACTIVATED in `faebot/index.js` paths[] (entry left commented out in place); pathBuilders + pools + template + recipes all checked in; pools at MVP-25. To activate: uncomment `'fae-cottage'` in paths[], scale pools (dwelling/setting 200, overgrowth 120, garden 150, lighting/atmosphere 100 — recipes have weighted sub-theme mandates ready). Gen-script gotcha: `gen-faebot-pool.js` takes `--target N` (grow-to-N); `--total` is silently ignored → append-instead-of-replace (55-entry pool from a stale 25 + default 30).
