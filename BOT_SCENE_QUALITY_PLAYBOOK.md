...
### BloomBot `flower-friends` — a SUBJECT-POOL repair is invisible while the template carries its own subject source (2026-09-23, Kevin)

The `flower_focal_cluster` pool (125 entries, 44 distinct flower line-ups, daisies in 100, pastel only) was rewritten in place to 125 distinct line-ups / 154 species / 9 colour families / 40 rich-register entries (tool `scripts/repair-flower-focal-cluster.js`, ledger `SEED_POOL_REPAIR_HANDOFF.md` §9). Forced renders of the NEW entries then showed the pool alone changed almost nothing, for three template reasons, each measured:

1. **The template injected a SECOND subject source after the entry.** `BLOOMBOT_FLOWER_FRIENDS` carried "COLOR PALETTE — STRICT ${sharedDNA.palette}" and "FLOWER SPECIES — STRICT ${sharedDNA.roster} … pick 3-5 species from the roster" BELOW the pool entry. On 1 of 3 forced renders Sonnet dropped the entry's five flowers and wrote the roster's (amaryllis / watsonia / tree peony / sea holly / barrel cactus). **Rule: when a path has a subject pool, the template must not also hand Sonnet a roster or palette for the same thing; two sources = a coin flip per render.** Removed; the entry is now the only flower source on this path. After: entry flowers led 8 of 8.
2. **The entry's own register words never reached Flux; the template's did.** Every entry ends "soft pastel watercolor register" / "rich clear watercolor register". Those words reached 0 of 3 Flux prompts; the template's "soft pastel magical light" reached 3 of 3. Fix was the STRUCTURE (output-order) block, not prose: it now asks for the entry's flowers with their colours first and the register as a fixed phrase ("soft pastel colours throughout" / "rich clear saturated colours throughout"). After: 8 of 8. Re-confirms "the output order reaches 100%, prose ~0%" (lessons 22/60/77) and "template wording with a fixed colour cast overrides pool variety" (ChibiBot heartwarming R2): the fix pattern is the same, tie the template's colour/light phrases to the axis with one example per value, and remove the fixed cast everywhere it appears (header, ambient, composition, lighting, structure), not just the header block.
3. **A pool-generation mandate had been living in the render template.** "COLOR DISTRIBUTION MANDATE — across the pool of 25, ~4 BLUE-DOMINANT, ~4 VIOLET …" was instructions for a seed generator, pasted into the per-render brief where Sonnet can do nothing with it except bias toward pastel. Cut. Audit templates for pool-scale language ("across the pool", "~N of 25").

**Colour-register findings on the renders (8 forced, flux-1.1-pro / ultra, looks rolled).** "Rich clear" lands where the family is a strong Flux colour: pink (ukiyo-e, magenta + deep violet), red (impressionist, crimson-dominant), multi (hollyhock spires, vivid). Blue and white "rich" read only mildly stronger than pastel. **Green rendered green on 0 of 3** (both registers, both templates): "vivid-lime euphorbia + chartreuse amaranth + deep-violet cyclamen + white alstroemeria + gold hibiscus" came back pink/white/violet; the companion species' own colours win. Same law as the flower×colour render-prior matrix above (you cannot recolor a wisteria yellow with a word): a GREEN-dominant line-up needs green-PRIOR species AND pale companions, or it is a pastel bouquet with a green title. The `flowers.json` `primary`/`versatile` matrix is the right validator for a pool's colour claims; the repair tool checked botanical reality only, not Flux's prior.

**4. The bot-wide density layer made the whole repair invisible, and the fix was the hanging-flowers move.** Even with 1-3 fixed, all 11 forced renders were the same edge-to-edge multicolour wall and Kevin asked "is everyone supposed to be as many colors and flowers as possible?" Three layers said yes regardless of the entry: the bot-wide `PROMPT_PREFIX` ("abundant blooms filling the entire frame edge-to-edge, dense overflowing bloom-mass", the FIRST tokens of every Flux prompt), `LUSH_HERO_MANDATE` in `buildBrief`, and the template's "FULL OF FLOWERS / filling the lower 50-65% / background of more blooms". Path-only fix: a `FLOWER_FRIENDS_MANDATE` hero branch (ONE composed cluster of the entry's flowers, open air around it, pollinator cast plainly visible), `promptPrefixReplaceByPath` (short positive anchor: "enchanted storybook garden vignette, one composed cluster of flowers with cute pollinators, open air around it"), `promptSuffixByPath` (drops "receding layers of more blooms"), and the STRUCTURE block naming the FOCAL POLLINATOR first. Same 8 entries re-rendered: **insects visible 8 of 8** (first insect at 33-41% of the prompt, was 52-59%), compositions now differ per entry (open-sky cluster / intimate single focal bloom / a giant butterfly on a red hibiscus), colour and register follow the entry. Kevin: "these are absolutely beautiful." No new medium key was needed (BLOOM_NEUTRAL's "filling the frame" clause is still in the prompt at words ~20-40 and lost to the prefix + Sonnet's composed scene). **Rule: before judging a subject-pool repair on renders, list every layer that fixes composition or density regardless of the entry (bot prefix, hero mandate, medium fragment, template hard mandates, output order) and neutralise them for that path first; a pool repair cannot be seen through a frame-packing mandate.** Precedent: hanging-flowers lesson 1 above; alpine-wildflower-meadow's BLOOM_NEUTRAL note in `index.js`.

**Pollinator position.** The insect was named at 52-59% of every prompt on all 125 original entries; naming the focal pollinator first in the STRUCTURE block put it at 33-41% and it rendered 8 of 8. Position law (lessons 60/65/77), confirmed again.

**Second path, same day: `flower-humming-birds` (both pools, `scripts/reseed/`).** The cast pool was 153 entries of 17 focal species × TWO behaviours (hover 101, sip 51) even though its gen recipe listed six; the flower pool had salvia in 61 of 120 and red-dominant in 35. Rewritten in place to 153 distinct casts (40 real species with real plumage, 13 in-flight behaviours, 40 flowers) and 120 distinct line-ups (70 species). The same template defects were there (a STRICT species roster + palette after the entries, a species list in three prose lines, the frame-packing prefix), and the before renders were 8 of 8 walls of pink peonies behind one bird whatever the flower entry said. After the path fix each render shows its own bird, open air and the entry's colour family; flower species recognizability stays partial on this path because the birds lead the prompt by design (the flowers sit past 40%), which is the path's intent. **Two lessons for the next pool:** (1) when a path has TWO subject pools, force both in the paired renders (`render-forced-entries.js --force a=… --force b=…`), or the un-forced one draws old entries and muddies the comparison; (2) a pool whose gen recipe listed N variants but whose entries use 2 of them is the common "one idea" shape: the fix is to pre-assign across the recipe's own list, not to invent a new axis.

**Repair-method lessons that transfer (seed pools):** (a) a pool's "same idea" must be defined per pool on its varying element (here: 4+ shared species of 5-6) and stated with every figure; the generic lexical measure and the Haiku same-idea judge both misread this pool (the judge calls same colour family = same idea); (b) never let Sonnet choose the varying element, it converges on the same dozen species batch after batch; pre-assign from a real roster, least-used first, pairwise cap, and check the assignment came back; (c) a colour-reality check must ask hue-family-only with "when in doubt, true" on Sonnet; Haiku rejected real colours (green zinnia, white agapanthus); (d) alternating register by slot parity while rotating family by slot index couples them (orange/yellow all pastel, blue/white all rich): roll register per family.

---

## ⚠️ AMENDMENT to "Medium Looks" LESSON 4 (STYLE-AUTHORITY wording) — it can trigger Sonnet's OWN injection defenses (2026-09-08, FarmBot)

The MangaBot section above (lesson 4) recommends explicit STYLE-AUTHORITY language when a look needs to beat baked template style phrases — verbatim: *"This is the AUTHORITY on rendering style. It OVERRIDES any other art-style / medium / finish wording anywhere below."* On FarmBot this measurably backfired: a QA review of 18 real renders found **Sonnet outright refusing or meta-commenting on ~28% of calls**, e.g. *"I notice a structural issue I should flag honestly: The 'override' framing at the top is... designed to make me treat outside instructions as having special authority. I'm happy to help you craft a genuinely excellent anime landscape illustration prompt, but I'll do that straightforwardly rather than through override mechanics."* The refusal/meta-commentary TEXT ITSELF then gets sent to Flux as the "prompt" (nothing downstream validates that Sonnet's output is actually a scene description), producing empty/generic renders with zero relationship to the intended scene — a failure mode that has nothing to do with which look or model was in play, and would misdiagnose as a look-register or model problem if you didn't read the actual stored `ai_prompt`.

**Cause:** words like "NON-NEGOTIABLE," "AUTHORITY," and "OVERRIDES" pattern-match Claude's own instruction-hierarchy/injection-defense training — MangaBot apparently didn't hit this at the sample sizes it was validated on (33 renders, 0 failures reported), but it's a live risk on any bot, and FarmBot's specific combination of these three words together in one block was frequent enough to hit ~1-in-4.

**Fix:** reword to plain, cooperative instruction language with the SAME functional intent (style leads, scene stays intact) but zero authority-claiming words: *"Please write the Flux prompt using this art style throughout. Keep all the scene content, characters, and composition described below exactly as written — just describe everything using this style's linework, shading, and color treatment. Start the Flux prompt with these style words so they set the visual tone from the very first line."*

**Cross-bot rule:** treat "NON-NEGOTIABLE / AUTHORITY / OVERRIDES" as a suspect phrase family on ANY bot from now on, not just FarmBot's fix. If a bot's render volume ever seems inexplicably generic/empty/off-brief independent of look or model, **read the actual stored `ai_prompt` for a refusal or meta-commentary pattern before touching pools or model config** — this is now a confirmed failure mode, not a hypothetical. Consider a periodic audit of `ai_prompt` values across the fleet for phrases like "I notice," "I want to be transparent," "structural issue," "prompt injection," "I appreciate the creative detail" as a canary.

---

## ⚠️ 2ND AMENDMENT to "Medium Looks" — a look-register entry must anchor on CHARACTER vocabulary, never time-of-day/weather/lighting (2026-09-09, FarmBot)

Two live FarmBot renders came back photoreal/painterly-Western instead of anime, on paths that had otherwise been consistently anime for the whole session. Root-caused (not guessed) via the FarmBot-scoped `sharedDNA.scenePalette` trace field (see the DLT-safe tracing mechanism, `index.js` `rollSharedDNA`) against the stored `ai_prompt`: both traced to look-register entries named **"Golden-hour anime landscape illustration"** and **"Moody rain-soft anime landscape illustration."** Neither of Sonnet's actual written Flux prompts contained the word "anime" (or any style token) anywhere — the entries' whole word-budget went to atmosphere ("richly painted... countryside," "luminous layered color") with "anime" mentioned exactly once and buried mid-sentence, versus the surviving entries' 3-4 repetitions anchored directly to character vocabulary (eyes, linework, cel-shading, proportions). A third entry, "Detailed anime background-art illustration," shared no time-of-day language but had the same background/environment-first framing — and also drifted painterly on a live render. All three were cut.

**Kevin's generalization, which is the actual rule going forward:** a look register exists to lock RENDERING STYLE/TECHNIQUE only. Time-of-day / weather / season / lighting words ("golden-hour," "twilight," "moody rain," "blossom-season") don't belong there at all — that's a separate, dedicated axis's job (this bot's `WEATHER_ATMOSPHERE` pool, tagged by season, already independently covers golden morning / blue hour / misty rain / blossom petals / sunset / starry night). A look entry that bakes in its own time-of-day doesn't just dilute the style signal (as above) — it can directly **contradict** whatever that path's own weather/season pick already set for the render (e.g. look says "Twilight" while the path's own weather axis rolled "misty spring morning"). Wanting more day/weather/lighting variety in a bot's renders is a real and legitimate ask — the fix is to enrich the dedicated weather/lighting axis, never to smuggle it into the look register.

**Cross-bot rule:** when writing or auditing ANY bot's look register, verify every entry (a) repeats a style-family word (anime/chibi/painterly/etc.) at least 2-3× anchored to character-rendering nouns (eyes, linework, proportions, shading), and (b) contains ZERO time-of-day/weather/season/lighting words. If a bot's renders drift off-genre only on SOME entries while others hold, don't assume the whole register is broken — trace the specific look via a scenePalette-style field (or read `ai_prompt` directly) and audit that one entry's word-budget against this rule before touching anything else.
...
## The KODAMA trap — an obscure creature with no Flux prior renders as an uncanny HUMAN-graft (2026-07-01, FaeBot forest-fairy-scene)

Kevin flagged a hearted FaeBot render: a bald porcelain HUMAN doll-head grafted onto a root-body wrapped in vines ("what the fuck is that?"). The rolled seed was a **kodama** (Mononoke tree-spirit): *"twig-slender kodama with translucent pale head... body merged with young saplings."* **Flux has no kodama prior**, so it rendered the descriptors at their nearest centroid — "translucent pale head" → bald human head; "body merged with saplings" → human form fused into a tree. Uncanny by construction. Same family as the DinoBot ~15-famous-dinosaurs collapse and the barreleye anthropomorphization trap, with a twist: here the collapse target is *a human*, which is maximally creepy on a beauty-first bot.

**The recipe sanctioned it** (the OceanBot law again): `gen-faebot-pool.js` mandated "~2 KODAMA / WOODLAND-SPRITE" per batch in TWO creature recipes and carried a kodama example touchpoint with the exact fatal phrasing; two `gen-seeds/faebot/*` scripts listed kodama in their species rosters with a ✅ teaching example. Net: ~44 kodama/graft-body seeds across 6 FaeBot pools, invisible at MVP, guaranteed to post under automation.

**Fix (all three layers):** purged 44 entries across 6 pools (tight classifier: kodama + round/pale/translucent-HEAD + dot/painted-eyes + body-merging-into-bark; porcelain-SKINNED dryads/sidhe with full faces and hair are the hearted register — NOT purged); stripped kodama from every recipe roster + distribution mandate + example; added an explicit ban: creatures must have a FULL beautiful face with real eyes and real hair — never bald/round-white-headed/dot-eyed/faceless, never a body fused into bark. Canary 3/3 clean.

**Cross-bot rule:** before seeding ANY creature species, ask "does Flux have a strong visual prior for this name?" If not, the words carry all the weight — and descriptors like "pale head / featureless / merged with X" collapse to a HUMAN centroid. Either describe such a being in fully concrete non-human terms (per the DinoBot body-plan-anchor lesson) or don't seed it. Watch for sibling traps: golems, wisps-with-bodies, "living statue" fae, anything "faceless" or "bald" on a creature bot.

## fae-cottage — "overgrown but not rundown, still OLD": calibrating an ancient-building path between three failure poles (2026-07-01, FaeBot — built, converged R5, DEACTIVATED pending Kevin)

New path from a Kevin-hearted render (flower-drowned stone chapel-cottage, amber gothic windows vs grey mist): ONE cozy fae dwelling as hero, 6 axes (dwelling/setting/overgrowth/garden×2/lighting/atmosphere) + VILLAGE_WILDLIFE critters on a 0.6 gate. Took 5 test rounds because "old fairytale building" sits between THREE failure poles, and Kevin's feedback bounced me off two of them:

1. **Present-day tidy** (R0/R1): log cabins, Tudor X-bracing with clean plaster, crisp coursed turrets, terracotta pots, bay windows — reads "real-world nice cottage." Kevin: "look too modern or not 'of the forest'."
2. **Organic moss-blob over-rotation** (R2): my fix — "GROWN, NOT BUILT," walls bulging, moss domes — produced hobbit-blobs. Kevin posted the source reference: the aesthetic is **dignified ancient STONE ARCHITECTURE whose lines still read** (gothic arches, gabled porch, bell-cupola), weathered pale masonry + mossy caps, the forest CLIMBING it (ivy sheets + ONE bloom cascade) — intact and loved.
3. **Derelict** (never shipped, ban held): overgrown ≠ ruin — no broken windows/collapse.
The working formula (template hard rule): "old = weathered stone + moss + the forest's embrace, NEVER decay, NEVER tidiness."

**Silhouette collapse — round-conical families all render as the same "cylinder with a pointy hat."** R3 shipped turret/brook-hut/dovecote rolls → 5/6 renders were identical round towers (Kevin: "cylindrical houses with pointy roofs seems to be every render"). Any round-massed family (turret, dovecote, moss-dome hut) collapses to the SAME Flux tower prior. Fix: every dwelling entry NAMES its massing explicitly (steep single gable / twin-gable / long low ridgeline / L-shaped wing / catslide / stepped two-level), conical-cylindrical capped at ONE entry per 25. Also reworded "cross-gabled"→"twin-gabled" (Flux injects literal crosses; it already puts cross finials on chapel-adjacent spires — recipes ban crosses, name the finial: verdigris ball / iron weathervane).

**Flux's cozy-cottage prior injects hardware the prompt never asked for:** iron wall lanterns by the door (~50% of renders), terracotta chimney pots, brass lever handles, gutter downpipes — verified absent from `ai_prompt`. Positive crowd-outs (chimney "of the same weathered stone as the walls", "rooflines end in bare mossy eaves", "the ONLY lights are the glowing windows") reduced but did NOT eliminate; the residual iron-lantern rate is a known cosmetic. Ban-lists made it worse (anti-baggage law) — never write "no lanterns."

**Keeper-pinning pass (2026-07-01/02, NOT yet converged):** Kevin pruned the feed to 21 keepers and asked to zero in. Tally of their `ai_prompt`s: golden/warm light 14/21, water 18/21, critters 19/21, dwellings led by moss-domed roundhouses (7) then turrets (4) — his earlier "cylindrical every render" complaint was batch SAMENESS, not round shapes (he kept 12 round ones; what he killed was TIDY, hardware-laden, dark, or austere). Dwelling/setting/lighting recipes re-pinned to that distribution (marked "PINNED 2026-07-01" in `gen-faebot-pool.js`), critter gate 0.6→0.75. **Verdict on the pinned R6+R7 batches: "still off from brand … too modern on a lot of them"** — the pin did NOT close the gap. Suspects for next session: Flux's injected hardware (path/wall lanterns, pots, finial spikes, crisp shingle courses) reading modern; Sonnet amplifying entries off-register; or the register gap lives somewhere not yet isolated — sit with Kevin and the 21 keepers side-by-side against the misses BEFORE another recipe edit.

**Status:** path DEACTIVATED in `faebot/index.js` paths[] (entry left commented out in place); pathBuilders + pools + template + recipes all checked in; pools at MVP-25 (hard-held until proven). iter-bot refuses deactivated paths — temporarily uncomment to test-render, re-comment after. To activate: uncomment `'fae-cottage'` in paths[], scale pools (dwelling/setting 200, overgrowth 120, garden 150, lighting/atmosphere 100). Gen-script gotcha: `gen-faebot-pool.js` takes `--target N` (grow-to-N); `--total` is silently ignored → append-instead-of-replace (55-entry pool from a stale 25 + default 30).

## fairy-swarm — LARGE fairy gatherings + the MODERN-PRIOR NOUN trap (2026-07-04, FaeBot — Kevin-confirmed 10/10 batch, scaled to production)

New crowd path (Kevin: "at least half a dozen per scene, purpose-built for large gatherings"). Two proven results:

**1. The ToyBot multi-figure recipe transfers to fairies verbatim.** 6-14 palm-sized fairies per render, first batch: crowds rendered correctly with zero single-hero collapse. The three load-bearing layers, all present: (a) `gathering_event` seeds OPEN with an active verb naming ONE shared object/event + 2-3 distributed sub-actions ("Raising a maypole… four hauling guy-lines while others steady the base"); (b) template composition lock as ABSOLUTE FIRST RULE ("AT LEAST SIX… spread across foreground and midground, NO single hero centered"); (c) a PLURAL `promptPrefixByPath` anchor ("a lively gathering of many tiny palm-sized fairies…" — FaeBot's first per-path prefix). Face-mush control: "the 2-3 NEAREST fairies have fully detailed faces; farther ones stay readable little figures" — held. `fae_troupe` is a GROUP-level axis (sketch 2-3 distinct leads + varied crowd) so the cast varies without 12 individual-fairy slots.

**2. THE MODERN-PRIOR NOUN TRAP (cross-bot, the batch's big lesson).** 2/6 first-batch failures, both seeded by MY OWN recipe mandate (the OceanBot law): "celebrating a tiny **wedding** under a foxglove arch" → Flux's wedding-photography prior rendered a FULL-SIZE MODERN bride + groom (belt, khakis) with the fairies as decoration; "tailoring a gown on a wishbone **dress-form**" → a literal tailor's MANNEQUIN in the forest. An event noun whose most famous image is modern/human-scale BEATS every fairy qualifier around it. The 4 keepers were all forest-craft nouns (birch-bark wish-boats, spider-silk pulley, maple-keys) — nothing to modernize. **Sibling trap found at scale-up: "conductor/conducting a chorus" (orchestra-tuxedo prior) — Sonnet re-derived it from the singing-circle theme even after "choir" was purged, so sweep SCALED pools again, not just the MVP.**

**The fix, all three layers (purging seeds alone regenerates the failure):** (a) rewrote the toxic seeds (wedding → blossom-festival lantern-lighting; dress-form → dressing a fairy volunteer; choir/conductor → singing rounds w/ mushroom-cap drum); (b) recipe now leads with a **FOREST-CRAFT LAW**: every object/garment/ceremony is fae-made of twig/leaf/petal/acorn/bark/spider-silk/dew, and every event noun must pass "what is the most famous image of this noun?" — if modern or human-scale, recast; garments are always petal-silk/leaf-cloth/woven grass; (c) template gained a positive-only TIMELESS FAE WORLD mandate (no ban-list, per the anti-baggage law). Validation: 4 + 10 posted renders, 10/10 clean prompts + Kevin: "love these." **Post-gen sweep regex to reuse:** `wedding|bride|groom|mannequin|dress-form|choir|conduct|belt|khaki|trouser|zipper|denim|camera|machine|plastic|jeans|birthday|christening` (+ the CJK range). Watch false positives: "dandelion clocks" (botanical), "a parade of pebble seats" (row-of).
...
## The character-path homogenization TRIAD — template lock + costume-in-action + costume-in-persona (2026-06-27, SteamBot all 4 character paths)

SteamBot's `airship-female/-male`, `sexy-steampunk-woman`, `steampunk-man` all rendered as "the same one person in the same outfit." Three independent homogenizers were stacked; fixing one alone does nothing. **Diagnose by READING the composed brief (`bot.buildBrief` in a tiny harness, no DB), not by eyeballing renders** — the failure is always visible in the prompt text.

1. **Template "OPENING/GENDER LOCK" that injects identity.** The template literally ordered Sonnet to *open with "a [ethnicity-coded] man/woman…"* and gave example ethnicities ("a Norwegian woman…", "a Yoruba man…"). Sonnet then **invents a nationality every render and ignores the skin pool**, AND the example ethnicities seed CLIP. Compounded by `${skin.split(',')[0]}` — the template kept only the first comma-chunk of the skin entry, dropping all face detail. **Fix:** delete the ethnicity lock; build the character from clean ATOMIC axes used IN FULL; "describe through visual traits only, never a nationality." (Real-world nationality labels are useless anyway — the model renders one generic person; diversify via pure VISUAL skin-tone/feature descriptors. Reaffirms [[feedback_no_realworld_diversity_in_fantasy_worlds]], but note steampunk = Victorian EARTH, so a FULL visual tone range porcelain→ebony is correct here, just label-free.)

2. **The ACTION/scene axis dictating COSTUME → overrides the outfit slot ("the captain-coat lock").** Action seeds read *"…long captain's coat billowing… officer…"* so Sonnet dressed EVERY render in a blue brass-button naval coat regardless of what the outfit pool rolled. The outfit pool had real variety; it just never got to drive. **Rule: the action axis describes ONLY pose + staging + props + sky/light — NEVER a garment.** One "fabric" is OK if it's the airship's gas-envelope; scan the pool (`/\b(coat|jacket|uniform|corset|gown|cloak|cape|epaulet|gloves|...)\b/`) and reword every hit. Same disease also lived as **crisis-busywork** (cannon-recoil, engine-valve-crisis — the "Titanic boiler-room" anti-pattern Kevin rejects); replace with composed iconic hero-shots, scaled one Sonnet call per iconic mold + the no-clothing recipe.

3. **The role/persona description carrying costume.** The `role`/`persona` pool entries embedded clothes ("AIR-MARSHAL … impeccable coat, gold braid epaulettes"; "Gentleman explorer in dust-flecked khaki, pith helmet"). Injecting the full string re-dressed the figure. **Fix:** either use the role TITLE only (`String(role).split('—')[0].trim()`) or reseed the persona pool to role + DEMEANOR only (zero clothing/props) — clothing comes from the outfit slot alone.

**The clean architecture that worked (mirror it for any human-character path):** split appearance into SEPARATE atomic axes — `skin` (tone, no nationality, no engine-grime/aging) · `face` (NEW axis: shape + features; add it when faces converge) · `eyes` (color) · `hair_color` · `hairstyle` (+ `facial_hair` for men). 25 entries EACH is plenty — the combinatorial product (25⁵ × actions) is millions of looks; padding atomic axes to 200 just makes near-duplicates. Only the scene-ish axes (action, persona) are worth scaling for feed freshness. Appearance brief = a labeled block ("Skin: … Face: … Eyes: …") used in full, NOT a comma-split. **Beware engine-grime/aging creep** in inherited pools (soot/oil/salt-stiffened/iron-grey/"tired"/"weathered") — it makes every character read old + grungy; strip it (subtraction, like the EarthBot hyperreal lesson). Men brief = "varied, handsome, MIXED ages" (a minority distinguished/silver, the rest prime; never seductive); women = "gorgeous + varied" with a full visual tone range. **Gotcha:** if the atomic pool entries already contain the word ("…blue eyes", "…black hair"), do NOT append " eyes"/" hair" in the template or you get "blue eyes eyes".

## Bucket-aggregated seeding — seed each sub-theme INDEPENDENTLY, then join (2026-06-28, ChibiBot creature-adventures)

The canonical way to seed a multi-setting "scene" pool (e.g. a path that spans amusement-park / beach / camping / fair / …): **generate each sub-theme in its OWN focused Sonnet call, then join them into one tagged pool** — never one big "cover all settings" call. Why: a single call unevenly starves some settings under cross-batch dedup, and each setting's seeds are higher-quality when Sonnet is focused on just that sub-grouping with its own examples. Mechanism: `scripts/lib/yumbotBucketGen.js` → `generateBucketScenes({ outPath, perSubTheme, subThemes:[{tag,blurb,example}], buildPrompt })` fires one `generatePool(append:true)` per sub-theme sized to that tag's gap, then tallies the distribution (asserts every tag hit `perSubTheme`). Idempotent — re-run at a higher `perSubTheme` and it fills only the gap. Entries are `{tags:[tag], description}`; at render time the path slot rolls across all tags uniformly (the composer maps tagged objects → `.description`). MVP at `perSubTheme:2` (covers every setting in ~25 seeds), render-check, then bump to 15+. This is the same equal-share rule as [[feedback_production_seed_equal_share_per_subtheme]] / weighted-subThemes below, applied to scene pools.

## Promoting a catch-all into a SHARED-FAMILY of bespoke multi-axis paths (2026-06-28, ChibiBot creature-outing)

When a single multi-setting catch-all path proves out and you want each setting to get more airtime + depth, **promote the strongest sub-themes to their own paths off ONE shared archetype + template** (don't write N bespoke templates — the FaeBot family lesson). ChibiBot: one `CHIBIBOT_CREATURE_OUTING` archetype/template (a band of chibi friends out doing something fun in a real-world place), reused by 6 per-setting paths (`creature-amusement-park`/`-beach-day`/`-camping`/`-snow-day`/`-county-fair`/`-birthday-party`); each path file just wires its own 4 bespoke pools — `activity` (setting-specific, **subject-agnostic** — "they/their", NO species, so the cast axis stays free) · `setting_detail` (pickN:3) · `prop` · `surprise_element` — while the **cast rolls combinatorially** from the shared `CUTE_CREATURES_UNIFIED` pool (`creature_group` pickN:3) + universal lighting/atmosphere. This multi-axis split gives way more variety than baking the cast into the scene (any 3 creatures × any activity × details × prop), and the setting reads from the activity+detail content so the template needs no per-setting label. Leave the weaker sub-themes in the original catch-all (trim the promoted tags out of its pool + its `subThemes` list). Scale the `activity` pool deepest (it's the headline); `25⁵`-style combinatorics mean detail/prop/surprise stay small. **Gotchas:** (a) keep activity phrases subject-agnostic or they fight the creature axis; (b) ambiguous ride words drift to literal objects ("spinning teacups" → a porcelain teacup → reword to "teacup-RIDE car"); (c) "distant crowd" renders as human-ish silhouettes on a no-humans bot — write "tiny chibi-ANIMAL visitors". New pool files must exist before the bot module loads them (`pools.js` `load()` runs at require) — run the gen BEFORE wiring/rendering. ChibiBot auto-derives look-enabled paths (`CHIBI_LOOK_PATHS`), so a new path is look-register-enabled the moment it's in `pathBuilders`.
...
## "The path looked great in dev but the automated feed degrades over time" — it's the LONG TAIL, not decay (2026-06-30, FaeBot fae-wilds-village)

Kevin's recurring frustration: a path ships looking perfect, then weeks of self-automated posting "gets worse." **There is no decay mechanism** — every post rolls fresh from the pools through Sonnet; nothing learns or drifts. The real cause is **selection bias on the combinatorial tail**: in MVP-25 dev you sample-and-approve the GOOD combos, so the path ships on its best face. Automation then rolls the FULL space (hundreds of combos) with **no human rejecting the bad ones**. Any combo that leans on a WEAK guardrail eventually rolls and posts — and that's the long tail you never saw in dev. **The cure is hardening the guardrails so the WORST combo is still acceptable, not tuning the best one.** When Kevin says "quality is going down over time," go hunt the weakest guardrail, not a regression.

**The concrete bug that surfaced it — Flux invents foreground figures as MODERN TOURISTS.** A hearted `fae-wilds-village` render (gorgeous elven cliff-village) had a modern family in t-shirts + cargo shorts on the foreground stone bridge. The Flux prompt named ONLY "two slender elves sharing tea on a high platform" — **nobody was specified on the bridge**, so Flux filled the path-to-a-castle with its single strongest prior: _present-day tourists visiting a castle_. Diagnosis steps that worked: pull the upload's `ai_prompt` (the figures Flux added were NOT in it → invented) + scan the inhabitant seeds (297/300 properly elf/fae-locked → NOT a seed problem). Root cause was a TEMPLATE gap: the three FaeBot village/city templates (`FAEBOT_WILDS_VILLAGE` / `FAEBOT_CASTLE_VILLAGE` / `FAEBOT_ELVEN_CITY`) — the architecture paths FULL of bridges/paths/stairs where Flux most wants to add tourists — were the ONLY FaeBot templates MISSING the human-figure lock the portrait/scene templates carry (`NO realistic non-magical humans`). The most at-risk paths had the weakest guard.

**Fix (the right lever for figure leaks): make Sonnet POSITIVELY populate the incidental figures, don't just negate.** flux-1.1-pro-ultra ignores negatives, and the guard lives in the SONNET meta-prompt anyway — so a positive instruction is what works. Added an `EVERYONE IN THE SCENE IS FAE — HARD RULE` footer to all three: every human-scale figure (foreground AND background, on every bridge/path/stair/platform) is a slender elf or tiny fae in flowing robes; if Sonnet places ANY figure on a bridge/path it must NAME it as a robed elf/fae — **never leave a foreground figure unstated (an unstated figure renders as a modern tourist in streetwear)** — then a short general negative (no present-day people/streetwear/tourists; no enumerated garment nouns, which would just seed them). **Generalizes: any path depicting human-scale architecture with paths/bridges/stairs near a recognizable real-world structure needs an explicit "name every figure + lock the wardrobe" rule, or Flux backfills modern people.** Template-only change (no seed/gen), apply-and-watch-the-feed.

## Out-of-genre SEED drift — Sonnet trope-reaching baked into the pools at gen time (2026-06-30, DragonBot)

Same day, same root mechanism, different surface. Kevin hearted-then-flagged two DragonBot renders: (1) a **pegasus** "dominance display" from `mythic-bestiary`, (2) a **tricorn-hatted pirate/colonial captain with a treasure map riding a white horse** from `fantasy-scene`. His words: _"why are characters in this genre wearing out-of-character items??? we get them setup and then they all start drifting into shit that isn't correct."_ **They do NOT drift at runtime — the off-genre seeds were baked in at SEED-GENERATION time.** When you ask Sonnet to gen "fantasy characters" / "dragon riders" / "sea warriors" at 200-scale, it reaches for ADJACENT tropes that statistically cluster with "adventurer" — pirates (tricorn/cutlass/flintlock), steampunk (clockwork/brass/pressure-gauge), Victorian-gothic (Bloodborne), Rococo (powdered wig). These land as a ~5-15% minority in the pool, invisible at MVP-N sampling, and only roll into the feed weeks later under automation. **A bot's brand is only as tight as its WORST seed × roll-probability-over-months.**

**Diagnosis that worked:** pull the hearted upload's `ai_prompt` → the offending noun is right there in the seed-composed text ("pirate captain… treasure map", "plunging pegasus"). Then run a fleet-wide regex sweep over every `seeds/*.json` for the off-genre token family and read the matches. **Two-tier classifier (don't nuke blindly):**

- **Greek-myth vs high-fantasy:** Greek-origin creatures that fully crossed into the CORE D&D Monster Manual as generic dungeon monsters (minotaur, hydra, chimera, manticore) read "fantasy" → KEEP. Creatures tied to a SPECIFIC named Greek myth / filed under D&D's Greek-themed Theros setting (pegasus, cerberus, cyclops, sphinx) read "Greek mythology class" → PURGE. Pegasus also tips twee/MLP.
- **Out-of-era items:** age-of-sail (tricorn, flintlock, cutlass-as-naval, naval coat, "pirate captain"), firearms of any kind (D&D/LOTR have none), steampunk/clockwork, Rococo (powdered wig), Bloodborne-Victorian register — all PURGE on a strict-high-fantasy bot. **Watch false positives:** "Arthur Rackham/Edmund Dulac/Walter Crane **Victorian** ink-and-wash" is an ART-STYLE/illustrator-era reference (classic fairy-tale illustrators — KEEP), NOT in-world Victorian content; "**corsair**" can mean a DESERT/Persian raider (Hammerfell, Arabian-Nights — fantasy, KEEP) vs a sea/naval pirate (PURGE) — disambiguate by the sea tells (salt-bleached, barnacled, aquamarine, naval). Kevin's call this session: purge ALL of it strict, including undead lich-admiral/skeletal-sea-captain `necro_subject` seeds (he overrode "these are fine D&D undead"). 73 seeds purged across the bestiary + 12 fleet pools.

**Mechanics:** these DragonBot pools load from `seeds/*.json` via `load('name')` at runtime — edit the JSON, no DB seed/deploy. Purge content-based (predicate on the seed text), NOT by index, and report before/after counts per file. **Cross-bot rule: after setting up ANY character/subject/outfit/creature pool, regex-sweep it for the off-genre trope families BEFORE shipping — Sonnet WILL have smuggled some in.** Token families to grep: `pirate|buccaneer|corsair|privateer|tricorn|flintlock|musket|cutlass|treasure map|brass.?button|colonial|naval coat|powdered wig|frock coat|steampunk|clockwork|victorian|bloodborne|pegasus|cerberus|cyclops|sphinx|cowboy|samurai|ninja|revolver|pistol`. Left intentionally (GoT/desert-fantasy-coded, defensible): Ironborn-sailor jerkins, cutlass-as-curved-sword weapon accessories, a dragon destroying a pirate galley (galley = prop the dragon wrecks), sea-elven hippocampus-cavalry armies.

## dragon-lore HERO-RELIC overhaul — a "stack the elements" template is a busy-collage engine (2026-07-03, DragonBot — Kevin-confirmed first batch)

Kevin flagged a hearted-then-hated `dragon-lore` render (the petrified-dragonfire hoard): _"i hate this particular one, what the fuck is it?"_ The old template's own doctrine caused it — a **MOVIE POSTER MANDATE ("stack 3+ visually-arresting elements", "every quadrant has something striking", "TILT TOWARD DRAGON-CODED EVERYWHERE")** + an **80% always-on phenomenon layer** + maximal 15-element scene seeds. When every quadrant is instructed to be arresting, no render has a hero — the OceanBot lean-axis lesson expressed at the TEMPLATE layer instead of the axis-count layer. Three compounding causes, all fixed and confirmed good on the first MVP batch:

1. **Template maximalism** → rebuilt around ONE hero: "THE RELIC IS THE HERO — ABSOLUTE FIRST RULE… instantly-readable silhouette owning 40-70% of the frame… One hero. Never a collage." Axes: `relic` (money axis, 25 relic families) / `setting` (a STAGE, "open enough to seat something the size of a castle") / `reclamation` ("ancient, never rundown" — the fae-cottage calibration as its own axis) / `human_trace` (tiny reverent scale-prover) / `sky_layer` / `palette`. Phenomenon gate 0.8 → 0.4.
2. **The bot-wide `scene_palettes` shared-DNA pool carries SCENE CONTENT NOUNS** ("Cumulus-white dragon sky citadel… Skyworld dragon aerial kingdom") — so every roll injected a SECOND scene under the "COLOR PALETTE" header (a sky-citadel palette on a cavern-hoard roll). Fix at the path layer: a path-bespoke `DRAGON_LORE_PALETTE` pool that is **colors-only by recipe mandate** ("NO nouns of things or places — color and light words only"). The shared pool was left untouched — it's read by the FROZEN artsy-girl/dragon-scene paths. **Cross-bot rule: a "palette" axis must be pure color/light language; scene nouns in a palette slot are a second-scene injection.**
3. **CJK corruption in Sonnet-genned pools:** 3/200 `scene_palettes` entries ended in stray Chinese characters (守 / 守护 — Sonnet pattern-completing "guardian" in Chinese). CLIP reads CJK tokens and they nudge renders toward East-Asian visual coding. **Add `[　-鿿]` to the post-gen sweep alongside the off-genre regex.**

Also: `surreal` removed from DragonBot's vibes bot-wide the same night (its "impossible color-pairings, arcane hallucinatory shifts" overrides any path's intended register) — don't re-add without Kevin. Old scene/architecture/surprise pools + registrations left on disk for revert.

## "Mundane / boring" renders — the SUBJECT-LESS PHENOMENON seed, and the recipe that mandates it (2026-06-30, OceanBot full pass)

Kevin hearted-then-flagged a `bioluminescent-night` render: a dark frame of glowing teal surf with scattered dots, no subject. *"We aren't here to render a snapshot of fuzzy glowing water — we're a creative AI app that posts images to INSPIRE and INTEREST people: ART."* **The rule this crystallizes: every scene seed needs a readable HERO — a creature, a named landmark/structure, or a dramatic story-moment. Glow / surf / aurora / mist / texture is the STAGE the hero stands on, NEVER the subject.** If a viewer could say "it's just glowing water / just an aurora / just some rocks," the seed has failed.

**Root cause was in the RECIPE, not just the seeds.** `gen-oceanbot-pool.js`'s `biolum_scenes` recipe literally had variety categories `B. MILKY SEAS`, `K. STORM-DISTURBED BLOOM`, `L. STARRY-SKY + GLOW` and a touchpoint reading "*...vast pale-aqua glow rising from the entire surface, no creature in frame.*" `polar_scenes` had `G. AURORA OVER OCEAN` and `H. POLAR LIGHT MOMENT (polar dawn / blue-hour)`. **The recipe TOLD Sonnet to write subject-less phenomena.** So purging the seeds is only half — you must rewrite the recipe's variety mandate to require a hero and demote phenomena to backdrop, or the next regen reintroduces the dullness. Biolum rewrite: "the bioluminescence is the LIGHTING; a living animal is ALWAYS the hero" → renders went from fuzzy water to a mako shark bursting through a glowing wave, a moon-jelly swarm, a manta over a glowing reef. Polar rewrite: "an animal OR a monumental ice formation is the hero; aurora/dawn may be a BACKDROP, never the subject" → a polar bear under an aurora (bear is the hero), not a bare aurora.

**Audit method (15 paths, ran 5 read-only sub-agents in parallel):** the regex "has-a-hero" classifier had false positives (flagged `tropical-fish` 90% dull because it didn't know species nouns like clownfish/parrotfish), so cross-path dullness needs AGENTS READING samples, not regex. Two failure CLASSES emerged: (1) **subject-less phenomenon** — concentrated in surface/weather/atmosphere paths (biolum, polar, ghost-ship-texture, coastal-generic); (2) **near-duplicate boilerplate + passive poses** — across the creature paths (whale "humpback breaching" verbatim ×9, kraken "erupting beside a galleon" ×10, mandarinfish ×15, reef-shark-cruising ×9). Dedup = group by a 6-significant-word key, keep ≤2 per group (removed 278 across 8 pools). Then `--target 200` regen with the hardened recipe refills with fresh hero-led variety.

**Two reusable levers added to recipes:** (a) coastal-power's "dullness" was generic unnamed coasts — fix was a NAMED-iconic-landmark mandate + roster (Nazaré / Cliffs of Moher / Big Sur / Reynisfjara…), not deletion (a wave detonating on a cliff IS dramatic; it just needs a name to be distinctive). (b) creature pools: "FAVOR active dramatic behavior (hunting / predation / breach / cleaning symbiosis / mass-event) over passive cruising / gliding / hovering / resting," and tropical-fish "reweight to bold charismatic subjects (lionfish / Napoleon wrasse / goliath grouper / giant moray face-fill), tiny subjects ≤20%."

**Flux gotcha found in the render check — "perched" makes a fish-BIRD chimera.** "Hawkfish **perched** motionless on staghorn coral" + an illustration medium rendered a fish with talons and a beak gripping a branch. Fish don't perch; "perched"/"branch" is bird-coded and Flux grafts avian anatomy (amplified by "Hawk"fish + illustration register). Fix: write "resting on its splayed pectoral fins," never "perched." Scrub existing seeds + guard the recipe. (Likely lurks in other reef/fish `*_camera_framing` pools too — sweep for `perched`.)

**Net OceanBot pass:** 447 dull/duplicate seeds purged+deduped across 10 pools, 7 recipes hardened, all pools regenerated to 200; biolum + a 5-path mixed sample render-verified on the feed. Coordination: another agent owned the `*_camera_framing` pools + `deep_wonder.json` concurrently — stayed in the scene/hero-pool lane and didn't touch theirs.

## Overused-POSE trope leaks fleet-wide — the recipe sanctions it, then it COMPOUNDS across axes (2026-06-30, StarBot "character on a wire")

Kevin flagged StarBot renders that keep showing a figure hauling/clinging/zip-lining on a wire/cable/tether (a hearted `spacewalk` render was an astronaut hand-over-hand on a guide-wire). Same root cause as the OceanBot dullness: **the gen recipe explicitly sanctioned the pose.** `spacewalk_eva_action`'s VARIETY MANDATE literally listed "hand-over-hand hauling along a guide-wire," "anchoring a cable to a truss," "reaching the apex of a tether arc"; `asteroid_mining_worker` had "an EVA team hauling a cable"; `character_action` had hardcoded touchpoints `RAPPELLING` + `ZIPLINING`; `busy_fleet_elements` had "EVA crews on tethers." Sonnet obeys — `asteroid_mining_worker`'s single "hauling a cable" item ballooned to **21 of 200** seeds.

**The amplifier — MOTIF COMPOUNDING across a path's axes.** A single spacewalk render rolls from several pools at once (action + nearby_structure + suit + suit_detail + theme). The tether was seeded in MANY of them (action "hand-over-hand," nearby_structure "a tether snaking off-frame," the theme's default body-language "tether drifting"), so even after de-wiring ONE axis the wire still shows up from the siblings. Same shape as the "girl with a bird" companion-compounding rule: when a motif is sanctioned in 3+ axes of one path, every render gets it. **Fix the motif in EVERY axis of the path, not just the obvious one** — and grep the broad family first (`wire|cable|line|tether|hand-over-hand|zip|rappel|cling|haul|grip|dangl|suspend`) to find all the axes carrying it.

**Diagnose precisely — broad regex over-counts.** A naive `wire|cable|tether` sweep hit 923 across 142 StarBot files (mostly legit set-dressing: cyborg body-wiring, space-elevators, cable-cars, snapped-cable hazards, hanging cockpit hoses). The TRUE "character posed on a wire" trope was ~64, found with a tight `verb-near-wire-noun within 45 chars` OR `hand-over-hand|zip-line|rappel` pattern. **Leave set-dressing** (a space elevator, a gondola, a cyborg's chest cabling, a snapped-cable danger beat, a loosely-trailing safety tether on a FREE-floating astronaut — that last one is realistic EVA and NOT the trope; the trope is the figure HAULING/HANGING on the line as the action).

**Two execution gotchas:** (1) GEN SATURATION — `character_action`'s theme was so constrained ("simple verbs, no props, grounded") that regen produced only entries that dedup-dropped against the existing pool ("Sonnet exhausted on theme," 0 added). When a pool is saturated, **author the new seeds directly** instead of fighting the gen. (2) regen can REINTRODUCE the trope even with a hardened recipe (it re-added one "climber-on-a-rope" to `landscape_anchor_entity`) — always re-scan the regenerated pools and add an explicit per-recipe guard. **Net:** 7 recipes hardened (wire items stripped + "NO character-on-a-wire" guards), 64 trope seeds purged across 9 pools, 6 recipe-backed pools regenerated, action pools also ENRICHED with fresh cinematic beats (spacewalk: solar-flare shield / coolant-ribbon / eclipse-silhouette / runaway-drone chase; character: carry-a-crewmate / calm-an-alien / plasma-blade-spin), 3-render spacewalk sample verified wire-free on the feed.
...
### DreamBot `far-eden` — happy fantasy ALIEN-WORLD vistas + the two-sibling-paths A/B (2026-06-28, NEW PATH ×2)

A third scene-as-hero DreamBot vista family, cloned off the `dreamscape` shape (NOT painterly `dream-spires`). Brief (Kevin): original, beautiful, ALLURING fantasy alien worlds — "brochure shots of alien worlds" you wish you could visit. Two reusable lessons:

- **The "happy register steer" — when a genre's Flux prior defaults to a TONE you don't want, ban the tone explicitly + tilt every axis toward the wanted tone.** Sci-fi/alien-world is the textbook case: Flux's prior skews dark/serious/ominous (derelict ruins, void-black skies, toxic glow, dystopian wastelands). Kevin wanted the opposite — joyful paradise. The fix is NOT one line; it's tone discipline across the whole stack: (1) a standalone **`HAPPY, BEAUTIFUL, ALLURING — THE #1 RULE`** template block (first, before the hero) that bans the dark register by name (no menace/horror/decay/derelict/dystopian/void); (2) every pool recipe carries the same `🚫 HAPPY ONLY` mandate (worlds inviting, cosmic sky = "WONDER never doom", terrain glow "warm never eerie/toxic", wonder-structure "elegant never grim/derelict", life "gentle never predatory"); (3) the `atmosphere` axis tilts BRIGHT/WARM (daylight, golden-hour, soft dawn — banning stormy/gloomy) so half the feed is sunny, not perpetual-twilight. A keyword drift-scan (`grep -iE '(man|woman|person|grim|derelict|ominous|menac|dystop|wasteland|ruins|dead)'` across the pools) verified 0 dark/human leaks at MVP. Generalizes to any genre with an unwanted default mood (horror-bot that must stay PG, a war theme that must read heroic-not-grim, etc.).
- **Two render LOOKS for one path = two sibling paths sharing ONE brain + a LOOK-NEUTRAL template; the MEDIUM is the only diff.** Kevin: "render both [hyperreal + painterly], you pick" → then "keep them both." The medium string (the per-path style prefix) is the dominant look lever and is locked per-path, so an axis can't swing painterly↔hyperreal (the medium prefix stays constant). The clean structure: write the template **look-neutral** on technique (say only "cinematic / vivid / luminous", NEVER "painterly storybook" or "hyperreal 3D" in the template body), then make two thin path files (`far-eden` + `far-eden-soft`) that share the SAME archetype + the SAME `eden_*` pools, differing ONLY in `mediumByPath` (+ its `promptPrefixByMedium`). Render the MVP half through each via `iter-bot --mode <path>`. This A/B's reproducibly (no mid-test config swap), and if Kevin likes both you just keep both — zero rework. If he'd picked one, you delete the loser path + medium (~6 index.js spots). **Gotcha:** the composer hard-rejects a `--mode` path not in `bot.paths` ("Path X not in bot.paths") — add the path to `paths[]` to render it even at MVP (safe: the cron dispatcher runs from committed `main`, so an uncommitted local `paths[]` add posts nothing).
- Same scene-as-hero shape as dreamscape: 5 always axes (`world` + `cosmic_sky` + `flora` ×2 + `palette` + `atmosphere`) + 4 independent gated conditionals (`terrain_glow` ~50% so half are bright-daylight, `life_accent` ~28%, `wonder_structure` ~30%, `dream_event` ~30%). AXIS-CLEAN; `cosmic_sky` is the signature money-shot axis (the dominating beautiful planet/rings every alien-world brochure is famous for). `world` recipe invents original NAMED alien worlds (Aelura, Bellaspar…) across 12 biome families. Code-only mediums (no DB row / no DLT); flux-1.1 pair (flux-dev + flux-2 family both ruled out after test renders); skip look-rotation + chaos + two-pass. Files: `paths/far-eden.js` + `far-eden-soft.js`, `DREAMBOT_FAR_EDEN` archetype + template, `EDEN_MEDIUM_HYPERREAL`/`_PAINTERLY` in shared-blocks, 9 `eden_*` pools (world 200 / flora 150 / others 90-120), recipes in `gen-dreambot-pool.js`. Commit `522d09c5`.

### DreamBot `hidden-conservatory` — stained-glass GREENHOUSE INTERIOR + the "keep the dark, mandate the vibrance" steer (2026-06-28, NEW PATH)

The same scene-as-hero machine pointed at an INTERIOR for the first time (ref: benmyhre "Stained Glass Greenhouses" — an overgrown garden inside a stained-glass cathedral, sun shattering through jewel-glass into rainbow prismatic light). The room is the hero, no character. Reusable lessons:

- **Scene-as-hero works for INTERIORS, not just landscape vistas — the architecture becomes the `world` axis.** Same 6-always + 3-gated shape as dreamscape/far-eden, just retargeted: `conservatory` owns the BUILDING (12 families — Gothic cathedral-glasshouse / Victorian iron / Art-Nouveau / domed palace / orangery / reclaimed chapel / palm-house / rose-window rotunda / Moorish / belle-époque / cloister / vaulted iron-and-glass hall), `glass_palette` owns the stained-glass COLOR, `flora` (×2) the overgrowth, `vantage` the interior CAMERA (down-the-nave / up-into-the-vault / corner bloom-bokeh), `atmosphere` the LIGHT+MOOD+AIR. Gated: `water_feature` ~35% (reflective surface doubles the color), `architectural_detail` ~40%, `ambient_life` ~22%.
- **The "keep the load-bearing dark mood, but MANDATE vibrancy in the signature elements" steer — the nuanced sibling of far-eden's full dark-ban.** Unlike far-eden (where the genre's dark prior was banned outright), here the moody-dark backdrop is *load-bearing* — it's the contrast that makes the prism-light pop. So the template does NOT ban dark; instead the `atmosphere` axis ROLLS the full range moody-dark↔bright (Kevin: "roll both"), while the #1-rule template block + every pool recipe mandate that the three things that matter — the STAINED GLASS, the FLOWERS, and the REFRACTED LIGHT — stay brilliant/vivid, and ban only true grim/horror/rotting-decay. Litmus: when a reference aesthetic *depends on* darkness for contrast, don't ban the dark — pin the vibrance to the specific hero elements and let a mood axis span the range.
- **Give the signature phenomenon its own always-on money-shot axis.** Every benmyhre shot leads with the prism refraction, so `light_effect` is a dedicated always-on axis (rainbow god-rays / colored floor-pools / sunstar-burst+lens-flare / prism bokeh / jewel-dapple / color-wash walls) — separate from `atmosphere` (which owns generic light-quality+mood). Same pattern as spacewalk's `visor_reflection` and far-eden's `cosmic_sky`: find the one detail the reference is famous for and make it its own axis.
- **MODEL-TEST FINDING — the bot-side ban is enforced TWICE; a `--model` force only beats the first gate.** `BOT_BANNED_MODELS` (nano-banana `google/gemini-2-image` + gpt-image-2) is stripped in `modelPicker.pickModel` AND again by a hard render-time guard at **`botEngine.js:~1515`** that swaps any banned `renderModel` → flux-1.1-pro-ultra. `iter-bot --model` sets `modelByPath` (bypasses the picker) but NOT the render-time guard — so forcing a banned model silently renders flux-1.1-pro-ultra instead. The `🎨 model=` log prints BEFORE the swap (it logged `gemini-2-image` while actually rendering pro-ultra) — **always confirm the ACTUAL model via `uploads.model` in the DB, never the iter-bot log.** To genuinely evaluate a banned model, temporarily env-gate that guard (`&& process.env.ALLOW_BANNED_MODELS_TEST !== '1'`), run with the flag, then revert — never commit the gate, the permanent ban stays intact. Also: **gpt-image-2 cannot render 9:16** (pins to 2:3 / 1024×1536), so its feed crop differs from every flux path — a real cost for a 9:16 wallpaper feed.
- Result of the test on this path: flux-2-pro/max = clean photoreal 9:16 upgrade; Nano Banana = warmer/softer, less prism-splash; GPT-2 = richest reference-match but wrong aspect. Kevin pegged the path to **flux-1.1-pro-ultra** (single-model lock, not an array). Code-only photoreal medium (no DB row / no DLT); skips look-rotation + chaos + two-pass. Files: `paths/hidden-conservatory.js`, `DREAMBOT_HIDDEN_CONSERVATORY` archetype + template, `CONSERVATORY_MEDIUM` in shared-blocks, 9 `conservatory_*` pools (arch 200 / flora 150 / others 90-120), recipes in `gen-dreambot-pool.js`. Commit `befd1ebf`.
...
**2026-07-02 amendment — the `earthbot_wow` fragment got the same treatment, at Kevin's own ask.** The 6/30 wow medium (488 chars: "jaw-dropping wow-factor … the most magnificent and fantastical … imaginable, luminous SATURATED color … almost too beautiful to be real", ultra-only model gate) made every render very bright + saturated. Kevin liked it but asked to dial back "just a tad — lush and very pretty and HD looking, but not so overblown … a tad edited and professional." Dial-back = remove ONLY the top amplifier stack, keep every real-magnificence lever, add "rich luminous color with a clean professional grade / crisp fine detail / polished and professional" (the professionally-color-graded ask in Flux terms). One variable changed (the DB fragment); the fine-art prefix, ultra gate, and palette pools untouched. Verdict: 6-render posted batch (epic-vista / reef-paradise / deep-forest), Kevin: "this looks good." **Calibration ladder for a real-world bot's register: documentary-restrained (old earthbot_photography) → lush-professional-graded (current, Kevin's pick) → maximal-amplifier (6/30 wow) → hyperreal (banned).** Current fragment + the 488-char revert copy live in `memory/feedback_earthbot_bold_wow_medium.md`.
...
### CRITICAL LESSON 7b — "cathedral / colonnade / column formation" in a FOREST subject pool renders the dead-symmetrical planted-avenue nave (2026-06-30, EarthBot deep-forest)

A Kevin-hearted EarthBot deep-forest render came back as textbook AI slop: two mirror-image rows of identical, smooth, perfectly-straight telephone-pole trunks with a central leaf "path" vanishing to a glowing point (a literal cathedral nave / planted tree-avenue). The rolled subject entry: "cathedral interior of ancient Bavarian beechwood, smooth silver-grey pillars rising sixty feet in dense column formation surrounding the frame."

Root cause: the `deep_forest_subject` pool was 84% saturated with architectural-symmetry vocabulary (`cathedral` x90, `column` x91, `colonnade` x68, `ranked` x58, `hall` x26, `symmetrical` x17, `pillar` x14 out of 200 entries). CLIP literalizes "cathedral interior" + "columns in formation" into cathedral architecture: a nave is two parallel ranks of columns + a central aisle leading to an apse. "Bavarian beech / bamboo grove" training data (real planted avenues) compounds it. "Cathedral forest" reads to a human as "awe-inspiring tall canopy"; to Flux it reads as "build me a nave."

Same law as the StarBot spaceship-cathedral lesson (search "Pool DNA dominates brief admonitions"): pool DNA dominates brief admonitions EVERY time. The 2026-06-18 fix had moved deep-forest onto a clean `FOREST_INTERIOR` template that explicitly bans "trail/path" and frames "inside the forest", but it left the subject pool's cathedral DNA untouched, so it kept rendering naves. A template/brief fix without a pool scrub is a half-fix that always loses.

**The fix (validated 6/6, 2 NatGeo-tier):** scrub the architectural-symmetry vocabulary from the SEED ENTRIES and replace with natural-irregular old-growth: trunks of wildly uneven girth at irregular spacing, some leaning/curving, a fallen mossy giant, an off-center dominant trunk, no central aisle, organic jumbled chaos. KEEP the good "looking straight up / forest-floor POV" framing + the species/biome. Done across all of EarthBot via `scripts/purge-earthbot-symmetry.js` (context-aware Sonnet rewrite; 36 pools, ~580 entries, 0 residual). Notably the rewrite did not just remove slop: "irregular/leaning/gnarled/off-center" gives Flux more characterful trees (sprawling root buttresses, serpentine roots).

**Poison vocabulary to ban in any FOREST subject/scene pool:** `cathedral`, `colonnade`, `column formation` / `dense column formation`, `cathedral spacing`, `cathedral-tall`, `ranked trunks` / `trunks in formation`, `nave`, `aisle`, `hall`, `avenue of [trees]` / `colonnade ... on both sides` (planted-avenue cliche, render as SCATTERED instead), `parallel/straight/orderly rows`, `evenly-spaced trunks`, `regimented`, `symmetrical trunks/rows/spacing`, `mirror-image`. NOT poison (do not over-scrub): `asymmetric` (the desirable opposite), `columnar basalt` / `basalt columns` (real geology), `ranks of hoodoos/spires` (real geology), "looking straight UP a single trunk" (good POV), clouds/satellites that are genuinely "evenly spaced."

**Diagnostic test:** a forest render that comes back as a symmetric corridor / planted avenue / cathedral nave with a central path: audit the subject pool for the poison list above; it is in the SEED, not the template.

### CRITICAL LESSON 7c — "lenticular DISC hovering motionless, matte silver-grey" renders a UFO MOTHERSHIP (2026-07-02, EarthBot cloud sweep)

A Kevin-hearted epic-vista render ("the cloud looks fake as fuck") had a colossal metallic-rimmed flying saucer parked over the peak. The rolled sky entry: "a motionless lenticular disc hovering smooth matte silver-white above the escarpment." **RIGID-OBJECT vocabulary on a cloud flips CLIP from meteorology to UFO**: `disc/disk`, `saucer`, `plate(s)` ("tiered like stacked ceramic plates"), `metallic / matte silver`, `hovering / suspended / motionless / perfectly still / sitting fixed`, `stack-of-three … disc tier`. The wow-medium's "almost too beautiful to be real" amplifier then scales it to mothership size. Same vocabulary-literalization law as StarBot's grab-rail→ship-deck and OceanBot's photophores-as-jewels→literal-gems — and the RECIPES sanctioned it (touchpoints literally said "smooth lenticular disk cloud hovering motionless"), so a regen would have reintroduced it.

**Weather-true replacement language (keeps the phenomenon, kills the saucer):** "smooth lens-shaped / almond-shaped CLOUD CAP with softly feathered edges, draped over / capping / clinging to the summit, visibly made of layered moist cloud"; a stacked lenticular = "several smooth layered bands of wave-cloud, one above the other, each softly feathered." Movement verbs "draped/capping/forming in the lee wave/streaming into ice-haze downwind" — never "hovering/suspended" (rigid-object verbs). Probe-verified on the exact flagged prompt: the metallic saucer became a fluffy, believably-lit (still dramatic) cap cloud.

**Sweep executed with `scripts/fix-earthbot-clouds.js`** (clone of the purge-earthbot-symmetry harness: regex classifier + Sonnet rewrite of ONLY flagged entries + re-validation): 94 entries across 17 pools rewritten, 13 recipe touchpoints in `gen-earthbot-pool.js` fixed, 0 residual. False positives excluded by design: "saucer magnolia" (a real tree), the SUN's disc (solar disc / disc clears the ridge — legit), "plateau", birds/water "motionless". Audited-and-KEPT as weather-true: morning-glory roll clouds, shelf/wall clouds, fallstreak holes (cavum), mammatus, nacreous/noctilucent, pileus, anvils — their entries describe real structure in real terms.

**Cross-bot rule:** any pool describing ATMOSPHERIC/OPTICAL phenomena must describe them as made of cloud/light/water — the moment an entry reaches for a rigid-object simile or noun (disc, plate, dome, ring, orb, "hovering motionless"), Flux renders the OBJECT. Sweep `disc|disk|saucer|plate|orb|dome|metallic|hovering|suspended|motionless` over any sky/phenomenon/atmosphere pool before shipping it.
...
### "Standing around" + "cheesy" character paths → Medium-Looks system (5TH PROVEN LOOKS BOT, 2026-06-30)

Kevin: SteamBot characters were "absolutely stupid — just steampunk characters standing around" and "boring AI slop in every render… cheesier than it used to be." Three stacked causes + the fix, in order discovered:

1. **The character templates ENGINEERED a static portrait.** `airship-female/-male` literally ordered `AIRSHIP HERO-SHOT / OWNING the frame / face BIG / fills 40-60% / telephoto compression (background hazed) / movie-poster framing`. Every lever = one posed face with the world blurred out (the [[bot-as-product environment collapse]] / MechBot static-hero-shot failure). **Fix:** rewrote them to a candid mid-task worker at ~20-35% inside a deep, IN-FOCUS, populated steampunk world (other crew/cargo/instruments), + a hard "aetheric airship in the SKY — never an ocean / wooden sailing ship" anchor (the male had drifted to literal age-of-sail naval romance). Validated via the prompts AND the images.
2. **The cheese is the MEDIUM, not the glam.** The character paths used `steambot-painted-woman/-man` = "golden-rim-light, jewel-tone, painted-FANTASY-COVER finish" (Frazetta/Brom/Vallejo) — that register renders romance-paperback covers by construction, while the SCENE paths' cleaner `steambot-hyperreal` read as real steampunk. **Kevin wants the women GLAM** — do NOT de-glam (I started to; he stopped me). Kill the cover-MEDIUM + the posing, keep the gorgeous faces.
3. **A single named "young woman" Flux-renders as a centered, full-makeup cover-model at large scale no matter how small you size her in the template.** A medium A/B (clean-cinematic vs a new grounded-matte medium) proved the medium alone doesn't fix it — the *men* de-cheesed completely on the grounded medium (a weathered goggled crewman), the *women* stayed glam pin-ups. So "shrink her in the template" loses to Flux's lone-female prior; the lever that actually delivered variety was the looks axis, not a single medium.

**Resolution = the Medium-Looks system (GothBot/MangaBot pattern), and it fixed framing + cheese + monotony at once.** Kevin: "since I liked multiple mediums, a looks system would do well here." SteamBot is a style-identity bot (like MangaBot) AND hybrid (character + scene + automaton, like ChibiBot) with a central `buildBrief` (like BloomBot/GothBot) — so: one **composition-neutral** `steambot_neutral` medium locks the steampunk-world identity (defers render-style + composition), a per-render **LOOK** rolls in `rollSharedDNA` and is prepended **once** in `buildBrief` with STYLE-AUTHORITY wording (beats the baked "lush painted key-art" phrases still in some templates), all 11 paths routed to neutral via `mediumByPath` derived from a `STEAMBOT_LOOK_PATHS` set. **6 hand-authored full-color steampunk render sub-styles** (grounded-gritty / cinematic concept-art / oil / ink-watercolor / gaslit-chiaroscuro / gouache-poster). Kevin cut the monochrome (engraving/blueprint/sepia) and muted-vintage (chromolitho) ones — "no monochrome, no muted vintage color." Validated across character/scene/automaton: looks render distinctly, identity intact, women glam, no cover-cheese. Commit `efcda4c2`. Files: `index.js` (`STEAMBOT_NEUTRAL_STYLE` + `STEAMBOT_LOOK_PATHS` + `lookOverride()` + wiring), `seeds/steambot_look_register.json`, `pools.js`. **This is the 5th proven looks bot — the first where the looks system was the fix for a CHEESE/COMPOSITION complaint, not just monotony.**

### steampunk-hybrid REVIVED — the variety lane (2026-06-30)
...
### CAMERA-FRAMING-AS-LAW pools rot fleet-wide — the deep-wonder fix was never propagated; a "hand holding a knife" pirates render exposed it (2026-06-30)

**Symptom.** Kevin hearted-then-flagged a `pirates` render (flux-1.1-pro-ultra): an **extreme close-up of weathered fingers gripping a cutlass hilt**, the entire rich pirate scene (careened galleon, treasure-strewn cove, fog bank) bokeh-blurred to mush behind it. Kevin: "a fucking hand holding a knife… why the fuck do we have lame ass seeds/scenes like this?" His broader complaint: *"we get the path in good shape, but over time when it posts on its own via automation, the quality starts going down."*

**Root cause = the SAME deep-wonder failure (CAMERA FRAMING is the LAW → the framing axis hijacks the composition), but on a SCENE path, and never purged from any path except deep-wonder.** Every OceanBot archetype template declares `CAMERA FRAMING (LAW)` / "The camera framing above is the LAW," and the template is a **Sonnet brief** — so Sonnet faithfully obeys the rolled framing as the dominant composition. The hearted slop came from `pirate_camera_framing.json` #13: `"CLOSE-DETAIL framing on rope-scarred hands gripping a cutlass hilt, the deck brawl softly dissolving in the deeper plane."` Sonnet promoted that to the lead → CLIP locks on "close-up of hands on a hilt" → macro hand, scene dissolved. The deep-wonder framing pool was hand-cleaned in June (gem-bokeh + barreleye fixes), but **the other 15 paths' framing pools were generated the same way and never audited** — they were full of `CLOSE PORTRAIT on [a face]`, `CLOSE-DETAIL on [hands/an object]`, `POLYP MACRO`, `CLOSE-UP ON GLOW DETAIL … texture filling the frame`, `BIRD'S-EYE TIDE-POOL` (empty pool, no hero).

**Why it "degrades over time" (the structural answer — this is NOT degradation):** the pools are 200 entries; the scene-destroying framings are a ~12% minority. **MVP-N render-testing (5 rolls) almost never hits them, so the path "looks great" at sign-off — but 4×/day automation eventually rolls every entry**, and the latent slop surfaces weeks later. The quality didn't drop; the bad entries were always in the tail, invisible at the sample size we validate at. **A path is only as good as its WORST pool entry × its roll probability over months, not its average entry at MVP time.**

**Fix shipped (2026-06-30):** audited all OceanBot framing + scene pools and purged **128 scene-destroying entries** across 7 pools — pirates 23, shipwreck-kingdom 25, lost-cities 24 (object-detail macros that dissolve the ruin/wreck), reef 24, biolum 25 (subjectless coral/glow texture macros + empty tide-pools), ghost-ship 5, deep_wonder scenes 2 (anglerfish "body dissolving entirely"). Classifier policy differs by path type:
- **Naval / SCENE paths** (pirates, ghost-ship, shipwreck, lost-cities, coastal, kraken): purge ANY framing that subordinates the layered scene to a face-portrait or body-part/object macro (`CLOSE PORTRAIT`, `CLOSE-DETAIL on hands/an object`, `EXTREME CLOSE`, `MACRO`, hands gripping/on X). **KEEP** `OVER-THE-SHOULDER` (scene visible ahead), `WATERLINE CLOSE-UP` (epic low shot, kraken+ship both in frame — "close-up" = camera position, not a detail zoom), `INSIDE-THE-BARREL` (epic surf tube), and all wide/low/aerial/through-rigging/side-profile framings.
- **Creature paths** (deep-wonder, whale, tropical-fish, reef, biolum, seaturtle): per the existing deep-wonder rule, **KEEP readable-animal close-ups** ("green sea turtle, shell filling the frame" / "lion's-mane jellyfish filling the frame" are the whole point); purge ONLY subjectless texture macros (`GLOW DETAIL`, `foam microstructure`, `POLYP MACRO`, `CLOSE CORAL DETAIL` with no animal), empty `BIRD'S-EYE TIDE-POOL`/`SAND-LEVEL MACRO`, "body dissolving entirely," and `as a [jewel/gem/bead]` similes.

Purge script: `scratchpad/oceanbot-purge.js` (dry-run prints every entry; `--apply` rewrites the JSONs in place — pools load from JSON at runtime, so edits are live with no DB seed or deploy). Pools land 175–198 entries; that's fine for random sampling. (Removed, not replaced — backfill to 200 with a hardened gen recipe later if variety needs it.)

**CROSS-BOT RULE (audit every bot with a framing-as-LAW axis):** any camera-framing pool whose template treats it as LAW must contain **ZERO entries that subordinate the hero scene/creature to a body-part, object, face-portrait, or pure-texture macro.** On SCENE paths a tight framing dissolves the layered scene (the entire product); on CREATURE paths a close-up is fine ONLY if a readable animal silhouette + identifying feature stays in frame. **These entries are invisible at MVP-N and only bite under automation — so framing pools must be audited as a SET, not sampled.** When a pool is generated by Sonnet at 200-scale, assume it contains this slop and scan for it before shipping (regex on `close-up|close-detail|close portrait|macro|extreme close|gripping|fingers|knuckles|glow detail|polyp macro|tide-pool|dissolving entirely`, then eyeball). The same rot almost certainly exists in other bots' `*_camera_framing` pools — sweep them.
